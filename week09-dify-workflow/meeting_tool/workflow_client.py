...
def http_failure_reason(exc):
    # 응답 원문 대신 알려진 원인만 반환합니다. URL과 인증 정보는 출력하지 않습니다.
    try:
    ...
    try:
        ...
        if "error code: 1010" in body.lower():
            return "edge_access_denied_1010"
        return "non_json_http_error"
    if not isinstance(data, dict):
        return "unclassified_http_error"
    messages = {
        "The app's API service has been disabled.": "app_api_disabled",
        "The app no longer exists.": "app_not_found",
        "The app's status is abnormal.": "app_status_abnormal",
        "The workspace's status is archived.": "workspace_archived",
    }
    ...
    if isinstance(code, str) and code in {
        "forbidden", "unauthorized", "trigger_workflow_service_mode_unavailable",
        "workflow_version_execution_not_allowed", "app_unavailable",
    }:
        return code
    return "unclassified_json_error"
    ...
    for entry in entries:
        ...
        try:
            ...
            results.append({"id": meeting_id, "status": "workflow_unavailable",
                            "error_kind": "http_error", "http_status": exc.code,
                            "reason": http_failure_reason(exc)})
            continue
            ...
            results.append({"id": meeting_id, "status": "workflow_unavailable", "error_kind": kind})
            continue
