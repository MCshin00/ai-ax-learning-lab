...
def summarize(path: Path, metadata_path: Path | None = None) -> dict[str, Any]:
    ...
    if metadata_path is not None:
        try:
            # Windows PowerShell 5.1의 Set-Content -Encoding UTF8은 BOM을 붙일 수 있다.
            metadata = json.loads(metadata_path.read_text(encoding="utf-8-sig"))
            raw_exit_code = metadata.get("codex_exit_code")
            if isinstance(raw_exit_code, int):
                codex_exit_code = raw_exit_code
            else:
                metadata_error = "codex_exit_code is missing or is not an integer"
        except (OSError, json.JSONDecodeError) as exc:
            metadata_error = str(exc)

    if codex_exit_code not in (None, 0) or turn_failures or runtime_errors:
        execution_status = "failed"
    elif event_counts.get("turn.completed", 0) > 0:
    ...
    return {
        ...
        "errors": [*parse_errors, *runtime_errors, *turn_failures],
        "parse_status": "valid" if not parse_errors else "has_invalid_lines",
...
def main() -> int:
    ...
    result = summarize(args.input, args.metadata)
