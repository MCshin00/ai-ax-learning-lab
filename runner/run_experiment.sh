...
if [[ $# -lt 4 ]]; then
  echo "usage: $0 RUN_ID PROMPT_FILE MODEL REASONING [PROFILE] [SANDBOX] [APPROVAL_POLICY] [TIMEOUT_SECONDS] [IGNORE_USER_CONFIG]" >&2
  exit 2
fi
...
APPROVAL_POLICY="${7:-never}"
TIMEOUT_SECONDS="${8:-1800}"
IGNORE_USER_CONFIG="${9:-false}"
...
python runner/parse_codex_jsonl.py "$RUN_DIR/events.jsonl" --output "$RUN_DIR/summary.json"
