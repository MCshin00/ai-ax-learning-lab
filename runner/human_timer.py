...
OUTPUT_ROOT = Path("experiments/timers")
...
def append_event(run_id: str, event: dict[str, object]) -> Path:
...
def main() -> int:
    ...
    if args.action == "start":
        ...
        save_state(state)
        append_event(args.run_id, {"type": "start", "at": now_iso, "activity": args.activity})
    ...
    if args.action == "stop":
        ...
        save_state(state)
        path = append_event(args.run_id, {"type": "stop", "at": now_iso, "activity": activity, "duration_seconds": round(duration, 3)})
