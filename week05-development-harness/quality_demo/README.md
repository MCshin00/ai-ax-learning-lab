...
환불·배치 업무는 완성해서 제공합니다. 이번에는 업무 요구에서 **작업 계약, 상태와 다음 행동, 실행 경계, 검사·피드백·복구·종료, 실제 시작 명령**을 도출해 AI와 개발 엔진 v1을 만듭니다. 학습자는 자기 엔진의 중심 구조와 진입점을 정합니다. 비교 예제의 인터페이스를 구현해야 실행되는 틀은 없습니다.
...
학습 결과는 자신의 요구 표현과 선택한 구조로 시작해서 실제 작업·검사·실패 대응·종료까지 이어지는 엔진입니다. 중심 구조를 AI와 만들며, 프로세스 입출력이나 환불 코드를 반복 구현하지 않습니다. 분석과 실행 근거는 주차의 `failure-recovery.md` 한 곳에 연결합니다.
...
두 구현은 같은 Job과 실행 보조 도구를 사용해 같은 업무 조건을 처리합니다. 학습자가 두 엔진을 반복 제작하는 과제가 아니라, 책임을 어디에 표현할지 판단하기 위한 실행 가능한 자료입니다.
