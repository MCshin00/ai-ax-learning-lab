...
학습 결과는 자신의 요구 표현과 선택한 맥락·지침·도구·검사를 실제 작업·실패 대응·종료까지 연결한 하네스입니다. 그 중심의 실행 구조를 AI와 만들며, 프로세스 입출력이나 환불 코드를 반복 구현하지 않습니다. 분석과 실행 근거는 [개발 하네스 학습 기록](../harness-learning.md)에 연결합니다.
...
요청 파일과 순서 함수로 연결한 구현은 [하네스 실행 안내](harness/run.md)에서 시작합니다. 실제 Codex 실행과 교육용 응답 실행의 명령·결과를 구분해 설명합니다.
