...
`Bm25Index`는 Lucene 9.12.3의 BM25와 한국어 분석기, `HybridSearch`는 RRF 순위 결합입니다. `SearchComparison.main`의 기본 실행은 실제 BM25이며 `--live`는 준비한 의미 색인으로 BM25·임베딩·하이브리드를 비교합니다. `--live --answer`는 각 검색 결과를 기존 실제 생성·출처 검사에 연결합니다. 작업 폴더는 이 프로젝트이고 자료·실제 연결 설정은 기존 실행과 같습니다. Day 3~4에서 같은 질문·조각·필터·문맥 수를 맞추고 결과를 해석합니다. Lucene의 `core`, `analysis-common`, `analysis-nori` 의존성을 IDE의 Gradle 동기화로 준비합니다.
