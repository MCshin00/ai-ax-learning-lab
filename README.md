...
구조를 볼 때는 작동한 예제에서 입력부터 결과까지 따라가고 왜 구성을 나눴는지, 원하는 동작을 바꾸려면 어디를 고칠지 살펴봅니다. AI의 설명을 실제 위치와 대조하고 작은 변경의 예상과 결과를 비교합니다. 작은 예제는 전체를 읽어도 되며 별도 분석 보고서를 만들 필요는 없습니다.
