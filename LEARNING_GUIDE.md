...
| 5 | 제공 검증·Hook으로 실패 복구 | 자기 요구와 기존 검사를 대조해 실행·보완 |
| 6 | SDK 호출 구조 이해와 자기 Tool 요구 선택 | 업무의 입력·결과·실패 처리를 정해 7주차로 연결 |
| 7 | 프레임워크를 이해하고 한 방식 선택 | Tool을 모델에 연결하고 후속 요구에 맞춰 수정 |
| 8 | 문서로 질문에 답하는 RAG 사용·구성 | 업무 요구에 따른 검색·근거 선택과 답변 확인 |
| 9 | Dify에서 자기 요구를 처리하는 Workflow 구성 | 입력·출력·실패 동작을 설정에 배분하고 실행 |
| 10 | 앞선 결과로 실제 작업의 도구 선택 | 선택한 구성에 후속 요구를 반영하고 판단 확인 |
| 11 | 제공 업무나 자기 프로젝트의 기능 완성 | 목적·구성·작동 결과와 변경 요구의 처리 확인 |
...
반복해서 놓치기 쉬운 명령 실패에 **대표 Hook 하나**를 연결합니다. 마지막에는 자기 프로젝트의 중요한 요구 하나를 골라 기존 검사가 실제로 확인하는지 대조하고 실행합니다.
...
## Day 5 — 자기 요구를 확인할 검사와 실행 방법 고르기

자신의 프로젝트에서 중요한 요구 하나를 고릅니다. 당장 별도 프로젝트가 없다면 앞선 주차에서 만든 코드나 MCP를 대상으로 합니다. 예를 들어 “존재하지 않는 상품 ID에 가격을 만들어 반환하지 않는다” 또는 “공백뿐인 제목을 선택한 규칙대로 처리한다”처럼 실제 동작과 연결합니다. 도우미가 기존 결과에서 후보와 대표 입력·기대 결과를 제안해도 됩니다.

선택한 입력과 기대값을 **기존 검사의 코드·확인 절차에 대조**합니다. 상품 조회의 정상 ID만 검사한다면 알 수 없는 ID의 오류 처리를 확인한 것은 아닙니다. 반대로 이미 해당 오류를 확인하는 검사가 있다면 그것을 재사용합니다. 빠진 조건이 있을 때만 AI와 기존 테스트에 그 사례를 보완하거나, 도구의 실제 입력·응답을 확인하는 기존 절차에 추가합니다. 검사 개수를 늘리기 위한 사례나 별도 검사 프로그램을 만들지 않습니다.

```text
내 프로젝트에서 중요한 요구 하나와 대표 입력·기대 결과를 함께 정해 주세요.
기존 테스트 또는 실제 도구 확인 절차가 그 조건을 확인하는 위치를 보여 주세요.
충분하면 그대로 재사용하고, 빠져 있다면 기존 검사에 필요한 사례만 보완해 주세요.
기대값은 선택한 요구에서 정하고 현재 구현이 내는 값에 맞춰 낮추지 마세요.
선택한 검사를 실제로 실행하고 입력·기대값·실제값을 대조하세요.
실패하면 원인 위치를 확인해 필요한 부분을 고치고 같은 검사로 재확인하세요.
검사 목적·확인 위치·실행 결과·수정 여부를 현재 failure-recovery.md에 기존 기록을 보존해 연결하세요.
실행 못 한 경우 이유와 남은 확인을 알리고, 실행했다고 기록하지 마세요.
새 Hook이나 별도 측정기·하네스는 만들지 마세요.
```

다른 프로젝트 폴더에서 작업한다면 요청의 기록 경로를 현재 주차의 `failure-recovery.md` 실제 위치로 바꿉니다. 검사가 이미 충분하고 코드를 수정할 필요가 없어도 **선택한 요구의 검사는 실제로 실행**합니다. 실패하면 구현·검사·실행 환경 중 어느 부분이 요구와 어긋나는지 판단해 고치고 같은 확인을 반복합니다. 표면에서 직접 확인하는 Skill·MCP 동작은 그 입력의 실제 결과를 근거로 사용합니다.

그다음 이 검사를 다음에도 실행할 방법을 고릅니다. IDE 테스트 실행, 프로젝트의 테스트 명령, 기존 CI, Hook 중 필요에 맞는 가장 간단한 방법을 사용합니다. 검사를 반복해서 빼먹는 문제가 없다면 새 자동화를 추가하지 않아도 됩니다. 기존 Hook을 연결했다면 실제 이벤트도 확인하되, Hook 로그로 기능 검사의 입력·기대값 대조를 대신하지 않습니다.

`failure-recovery.md`에는 선택한 요구, 기존 검사의 확인 위치와 충분·누락 판단, 실제 결과, 보완하거나 고친 부분, 유지할 실행 방법과 이유를 이어 씁니다. 이미 있는 검사 출력·코드·실제 도구 응답은 해당 위치로 연결합니다. 시간 측정·상태 전이 로그·증거 manifest를 따로 만들지 않습니다.

**남길 결과·완료 판단:** 선택한 요구의 입력·기대 결과를 기존 검사에 연결해 실제로 확인했고, 빠진 검사는 필요한 만큼 보완했다면 Day 5를 마칩니다. 실패가 있었다면 같은 검사에서 복구 결과까지 연결합니다. 새 자동화를 추가하지 않은 경우도 검사 실행은 포함하며, 실행하지 못했다면 남은 항목을 표시하고 Day 5를 완료했다고 기록하지 않습니다.
...
- [ ] 자기 프로젝트의 중요한 요구 하나를 기존 검사의 입력·기대값과 대조해 충분한지 판단하고 실제로 실행했습니다.
- [ ] 필요한 경우에만 기존 검사에 누락 사례를 보완하고, 실패가 있었다면 같은 검사로 복구를 확인했습니다.
...
프로젝트가 이미 제공하는 검증 명령부터 사용하고, 자기 요구를 확인하는 사례가 있는지 대조합니다. 누락된 조건만 기존 검사에 보완하고 반복해서 빠뜨리는 단계만 기존 도구로 연결합니다.
...
<!-- MODULE:11 START -->
# 11주차 — 기존 기능을 업무 요구에 맞게 완성하기

이번 주에는 앞선 Skill·Tool·Workflow·코드 가운데 필요한 기능을 **새 업무 요구에 맞게 완성하고 실제 입력으로 사용**합니다. 10주차의 결과를 그대로 이어도 됩니다. 개인 프로젝트가 없다면 아래 제공 업무를 사용합니다. Skill Creator·AI·기존 SDK로 구현할 수 있으며 새 서비스나 모든 기술의 조합을 만들 필요는 없습니다.

글·보고서는 이 기능의 용도·설계 판단·실행 결과를 전달하는 매체로 남길 수 있습니다. 이론을 요약하거나 문장만 정리한 결과로 설계·제작 목표를 완료 처리하지 않습니다. 이미 만든 기능을 가져오는 경우에도 이번에 달라지는 요구와 그 요구를 처리한 실제 구성을 확인합니다.
...
입력을 받아 반복 사용할 수 있는 작은 기능과, 새 요구를 어떤 구성으로 처리하고 무엇을 확인했는지 보여 주는 근거를 남깁니다. 설명과 판단은 기존 프로젝트 README 또는 학습 저장소 루트 기준 `week11-portfolio-build/project-notes.md` 한 곳에 이어 씁니다. 기록 파일은 이 주차 폴더에 두고, 실제 수정은 선택한 프로젝트 폴더에서 진행합니다.

AI가 목적·범위·검증 방법과 기록의 초안을 제안하고 학습자가 실제 근거와 대조해 수정해도 됩니다. 매일 새 문서를 만드는 대신 실행 결과·diff·원문이 이미 있는 위치를 연결합니다.

## 바로 시작하기

앞서 만든 기능에서 다음 사용자가 새로 필요로 하는 동작을 정합니다. AI에게 후보와 구성 초안을 받아도 됩니다. 사용할 사람, 받을 입력과 자료, 원하는 결과, 처리할 수 없는 경우를 구체적인 예로 확인합니다. 정해진 문서 양식을 먼저 완성하지 말고 기존 노트의 짧은 결정과 실제 구현을 이어 갑니다.

### 개인 프로젝트가 없을 때 사용할 제공 업무

**회의 후 수신 팀에 필요한 작업과 확인 항목을 인계하는 기능**을 만듭니다. 10주차에서 만든 결과가 있으면 재사용합니다. 없다면 기존 정리 Skill이나 9주차 Workflow에서 시작하고, Skill이 적합하면 Creator로 초안을 만들어도 됩니다.

| 정할 범위 | 제공 업무의 요구 |
|---|---|
| 사용자와 목적 | 회의 진행자가 팀별로 담당 작업을 전달하고 미정 항목을 다시 확인함 |
| 기본 입력 자료 | 학습 저장소 루트 기준 `week10-ai-development-methods/README.md`의 Day 4에 있는 합성 메모·수신 팀과 기대 동작 |
| 기본 결과 | 수신 팀의 작업·담당자·기한과 담당 팀이 미정인 확인 항목. 다른 팀의 작업을 섞거나 미정인 팀을 추정하지 않음 |
| 이번에 추가된 요구 | 취소된 작업을 실행 목록에서 제외하되, 해당 팀의 취소 항목으로 구분해 남김 |
| 처리할 수 없는 경우 | 수신 팀·원문이 없으면 필요한 입력을 요청. 무엇이 취소됐는지 불분명하면 임의로 작업을 없애지 않고 확인할 질문을 남김 |

추가 요구에 사용할 합성 자료는 다음과 같습니다. 두 번째 입력도 같은 원문을 다른 답변으로 고친 예시가 아니라 기능에 보낼 별도 입력입니다.
...
확인이 필요한 별도 입력:
수신 팀은 결제팀이다. 환불 안내 수정과 결제 배너 교체 중 한 작업을 취소했는데
어느 작업인지는 회의 메모에 남아 있지 않다.
```

첫 입력에서는 환불 안내 수정은 남기고 배너 교체는 실행 목록에서 빼며 취소 항목으로 구분해야 합니다. 계정팀 업무를 결제팀으로 배정하면 안 됩니다. 두 번째 입력에서는 취소 대상을 추정해 삭제하지 말고 확인이 필요하다고 알려야 합니다. 이 자료와 기대는 과제이며 실행 근거로 복사하지 않습니다.

다른 기능을 선택한다면 이 업무를 추가로 만들 필요는 없습니다. 예를 들어 앞선 조회 Tool의 입력 조건이나 정보 부족 처리 요구를 바꿀 수 있습니다. 학습자가 선택한 후속 요구가 실제로 처리 규칙·자료 연결·입출력·실패 동작 가운데 무엇을 바꾸는지 설명하고, 동작으로 확인하면 됩니다.

사용할 데이터의 공개 가능 여부와 비용·외부 변경 경계를 확인한 뒤, 기존 도구·예제·Skill로 가장 작은 기능부터 만듭니다. 완성된 기능은 같은 방식으로 다음 입력에도 다시 사용할 수 있어야 합니다. 오늘 한 번 사용하기에 범위가 크면 기존 기능을 남기고 이번 추가 요구를 좁힙니다.
...
| 제작 결과의 기술 사례 설명 | 작동 기능의 결과와 출처를 정리하는 도구 | 기능 실행과 설계 판단을 뒷받침하는 근거 |
...
|---:|---|---|
| 1 | 기존 기능과 추가 요구를 정하고 첫 동작 만들기 | 재사용할 실제 구성과 입력·출력, 이번에 달라지는 요구 |
| 2 | 결과를 사용하며 핵심 문제 찾기 | 근거를 붙인 수용·거절 이유, 우선 수정과 수용 기준 |
| 3 | 추가 요구를 처리할 구성을 선택하고 기능에 반영 | 실제 구성과 선택 이유·예상·새 요구의 실행 결과 |
| 4 | 실제 사용 조건에서 다시 확인하기 | 수용 기준별 실제 근거, 남은 실패·미검증 범위 |
| 5 | 12주차에 넘길 결과와 한계 정리 | 결과를 여는 안내와 12주차에서 먼저 확인할 일 |

### Day 1 — 첫 결과 만들기

도구에 사용할 사람과 입력, 기존 기능과 추가 요구를 보여 주고 가장 작은 동작을 요청합니다. AI나 Creator가 기능·지침·설정의 초안을 만들고 학습자가 실제로 사용합니다. 이미 기본 기능이 있다면 그 기능에 같은 입력을 보내 현재 가능한 동작과 추가로 처리할 요구를 구분합니다. 첫날에는 기본 동작부터 확인하고 Day 3에서 추가 요구 반영을 마쳐도 됩니다.
...
```text
주차 README Day 2~3의 가상 릴리스 메모는 기능 출력의 수용 판단과 수정 위치를 설명하는 자료입니다. 가상 출력을 내 실행 기록으로 쓰지 말고 실제 기능의 입력·출력과 구현을 근거로 판단을 도와주세요.

내가 사용할 작은 결과를 만들고 개선하도록 도와주세요.

- 사용할 사람과 실제 작업: {현재 생각 또는 방향이 필요한 이유}
- 활용할 기존 결과·도구: {자료 위치}
- 추가 요구와 오늘 확인할 동작: {제공 업무 또는 자기 요구, 초안 제안 가능}
...
지정한 단계의 실제 기능을 만드는 데 필요한 작업을 진행해 주세요. 개인 프로젝트나 아이디어가 없으면 주차 README의 팀별 인계·취소 처리 요구를 사용합니다. 내가 판단할 수 있게 입력·자료·처리·출력·실패 동작과 구성의 초안을 제안하고 기존 도구로 구현해 주세요. Skill은 Creator로 만들거나 수정해도 됩니다. 이미 작동한 기능은 재사용하고 같은 기능을 새로 복제하지 마세요.

이미 결과가 있으면 입력과 출력에서 같거나 달라진 부분을 짚고, 그 차이를 처리한 지침·함수·설정·노드·자료의 실제 위치를 보여 주세요. 글·보고서는 이 기능의 설계 판단과 실행 근거를 설명하는 용도로 보완하세요. 그 근거에서 수정이 필요한 이유와 유지할 부분을 설명하되, 기록에 없는 설계 의도나 원인은 추정으로 구분해 주세요. 작은 결과물은 전체를, 큰 결과물은 대표 사용 경로와 관련 연결을 살펴봐도 됩니다.

Day 2에서는 실제 동작의 수용·거절 이유와 추가 요구를 받아들일 기준을 남겨 주세요. Day 3에서는 그 요구를 맡길 지침·함수·설정·자료를 선택해 구현하고 새 입력에서 예상과 실제를 대조해 주세요. 이미 요구를 충족하면 유지 이유와 실행 근거를 확인하세요. Day 4에서는 처음 구성을 만들 때 쓰지 않은 입력과 필요한 실패 사례도 확인하고, Day 5에서는 기능을 다시 사용할 방법과 다음 확인을 안내에 연결하세요. 사용할 수 없는 도구나 확인하지 못한 핵심 동작은 미완료로 표시하세요.
...
**Day 1에서 남길 것:** 기록에 사용 목적·재사용할 기능과 추가 요구·실제 구성 위치를 적고 기본 기능의 입력·출력을 연결합니다. Skill 호출이나 Workflow 실행처럼 본인이 그 구성을 다시 사용해 결과를 얻을 수 있으면 Day 2로 갑니다. 설명 초안이나 생성 완료 보고만 있고 기능을 사용하지 못했다면 첫 실행이 남은 상태입니다.

### Day 2 — 실제 결과에서 범위 정하기

Day 1의 기능을 사용하고 원자료·추가 요구와 대조해, 사용할 수 있는 부분과 우선 고칠 부분을 정합니다. **이번에 남길 것은 근거가 붙은 수용·거절 판단과 수정 기준**입니다. 아래 해설처럼 어떤 출력이나 동작을 보고 판단했는지 기존 기록에 연결합니다.

#### 해설 — 같은 결과에서 사용할 부분과 고칠 부분을 가려내기

아래 릴리스 메모 사례는 정리 기능이 만든 출력의 정확성을 판단하는 방법입니다. 설명 문단만 작성해 제작을 대신하는 과제가 아닙니다. 자기 기능에서도 입력·출력의 차이를 실제 지침·설정·코드와 연결합니다.
...
기록에서 현재 기능의 수용 판단과 추가 요구의 기대 동작을 찾을 수 있으면 Day 2를 마칩니다. 제공 업무에서는 취소된 작업과 취소 대상이 불분명한 입력을 어떻게 구분해야 하는지 정합니다. 추가 요구를 이미 처리한다면 설정을 유지할 이유를 검토하고 Day 3에서 새 입력의 실제 동작을 확인합니다.

### Day 3 — 추가 요구를 처리할 구성을 선택하고 반영하기

Day 2에서 정한 추가 요구를 실제 기능에 반영합니다. 입력·자료·지침·함수·출력·실패 처리 중 무엇이 이 요구를 맡아야 할지 선택하고 AI의 설계·구현 초안을 검토합니다. **남길 결과는 요구를 반영한 실제 구성과, 선택 이유·예상·새 입력의 실행 결과**입니다. 다음 해설은 출력 오류를 발견했을 때 같은 방식으로 수정 위치를 찾는 예입니다.
...
선택한 기능의 실제 파일·화면에서 입력부터 출력까지 따라갑니다. 제공 업무라면 취소 정보를 어느 단계에서 읽고, 실행 목록·취소 항목·확인 질문을 어떻게 나눌지 선택합니다. AI에게 구성 대안을 제안받아도 되며 자기 요구와 실제 입력으로 수용·수정합니다. Skill이면 Creator로 지침을, Workflow면 필요한 설정·연결을, 코드면 관련 함수를 바꿀 수 있습니다. 모든 역할을 별도 파일이나 노드로 만들 필요는 없습니다.

바꿀 위치와 예상 변화, 함께 영향을 받을 기존 동작을 정한 뒤 기존 도구나 AI로 구현합니다. 추가 요구의 입력을 실행하고 원문·기대와 대조합니다. 이미 요구를 충족하는 구성은 유지하되 그 이유와 실제 입력의 결과를 확인합니다. 제공 예제나 AI의 제안이 곧 자기 실행 결과가 되는 것은 아닙니다.

추가 요구를 어느 구성에 반영했는지, 왜 선택했는지, 예상과 실제 결과가 어땠는지를 기존 기록에서 찾을 수 있으면 Day 3을 마칩니다. 관련 diff·설정·실행 결과를 연결하고, 남은 실패는 Day 4에서 이어 확인합니다. 문구를 정리했거나 설정을 만들기만 하고 실행하지 않았다면 기능 확인은 남은 상태입니다.

### Day 4 — 약속한 결과를 확인하기

처음 구성을 만들 때 쓰지 않은 입력에서도 약속한 기능이 작동하는지 확인하고, 필요한 정보 부족·실패·반례를 함께 봅니다. 제공 업무라면 다른 팀·작업으로 바꾼 메모에서도 팀별 선택과 취소 처리가 유지되는지, 취소 대상이 불분명할 때 임의 삭제하지 않는지 확인합니다. 사례 수는 주장과 위험에 맞춥니다. 작은 문서 도구에 운영 실험을 요구하지 않지만, 실제 외부 쓰기를 한다면 승인·권한·중복 부작용을 확인해야 합니다.

IDE의 실행·테스트 기능이나 현재 도구의 미리보기를 먼저 사용합니다. 명령이 필요하면 해당 프로젝트 README의 Windows PowerShell과 macOS·Linux·WSL 명령을 실제로 확인해 남깁니다. 글·보고서로 전달한다면 기능을 실제 사용할 위치·호출 예와 설명에 사용한 근거를 연결합니다.

AI의 테스트 요약을 실제 결과와 대조합니다. **Day 4에서 남길 것은 약속한 기능과 추가 요구에 대응하는 실제 입력·결과·구성 위치와 수용 판단**입니다. Fake 응답으로 확인한 배선과 실제 모델·서비스 동작을 구분합니다. 실행하지 못한 핵심 동작은 `NOT_VERIFIED`, 실행했으나 기준과 다르면 `FAIL`로 적습니다. 어떤 기준을 확인했고 무엇이 남았는지 구분되면 Day 5에서 전달할 범위를 정합니다.
...
- 누가 어떤 작업에 쓸 기능이며 이번에 어떤 추가 요구를 반영했는지
...
| 선택한 실제 프로젝트 | 작동 기능의 코드·Skill·Workflow와 필요한 자료를 두는 위치. 기존 프로젝트를 복사하지 않고 확인한 위치·버전을 연결 |

새 프로젝트를 이 주차 안에 만들었다면 기록에 실제 폴더 이름과 핵심 파일의 역할을 짧게 소개합니다.

## 완료 기준

- [ ] 자기 목적 또는 제공 업무의 사용자·입력·자료·결과·처리할 수 없는 조건을 판단했습니다. AI의 설계 초안을 검토해도 됩니다.
- [ ] 앞선 기능을 재사용하거나 필요한 작은 기능을 만들어, 실제 도구에서 반복 사용할 수 있습니다.
- [ ] 추가 요구를 맡길 지침·함수·설정·자료를 선택한 이유와 실제 반영 위치를 설명할 수 있습니다.
- [ ] 새 요구와 처음 구성에 쓰지 않은 입력, 필요한 정보 부족·실패 사례를 실제로 확인했습니다. 기대와 다르면 원인을 찾아 고쳤습니다.
- [ ] 원래 지원 범위와 달라진 사용 방법, 실제 근거와 남은 한계를 연결했습니다.
- [ ] 다음 사람이 기능을 사용할 경로와 12주차에서 보완할 것을 남겼습니다. 글·보고서는 그 기능의 제작·실행 근거를 전달하며 이론 요약만으로 제작을 대신하지 않습니다.

선택한 기능의 제작 완료가 앞선 모든 기술의 숙련을 뜻하지는 않습니다. MCP·Tool Calling·RAG 등의 확인 범위는 해당 주차에 남긴 실제 근거로 판단합니다.
<!-- MODULE:11 END -->
...
11주차에서 완성한 작동 기능과 짧은 사용·검토 안내를 남깁니다. 코드라면 실행 방법, Skill이라면 실제 호출 예, Workflow라면 여는 방법이 중심입니다. 글·보고서로 전달하면 그 기능을 사용할 위치와 설계·실행 주장의 근거를 연결합니다. 기존 README·문서를 보완하거나 학습 저장소 루트 기준 `week12-portfolio-evidence/handoff.md` 한 곳에 연결합니다. 11주차에서 사용한 실제 프로젝트를 이어서 사용하고, 이 주차 폴더에는 필요한 전달 안내만 둡니다.

Day별로 확인한 문제·수정·한계를 이 안내에 이어 반영합니다. 기존 실행 결과와 출처는 링크하고, 매일 새 기록 파일을 만들지 않습니다.

공개·배포는 선택입니다. 작동하는 로컬 기능·Skill·Workflow를 비공개로 전달해도 됩니다. 기술 글이나 사례 연구는 기능과 실제 제작 근거를 전달하는 매체로 사용할 수 있으며, 설명만 존재하는 상태를 기능 제작 완료로 표시하지 않습니다.

## 바로 시작하기

1. 현재 결과물과 사용 안내를 엽니다. 설명이 없다면 AI에게 기존 결과에 근거한 짧은 초안을 요청합니다.
2. 안내만 보고 대표 입력 한 건으로 기능을 사용합니다. 전달 글이 있다면 기능의 위치와 그 실행을 설명한 주장의 근거까지 따라갑니다.
...
| 제작을 설명하는 글·보고서·notebook | 기능 사용 위치와 주장→실행 근거를 따라 읽기 | 출처·인용·설계 판단의 정확성 |
...
11주차의 기능과 전달 안내를 열고, 안내만 따라 대표 입력과 추가 요구의 동작을 확인합니다. **이번에 남길 것은 확인한 경로·실제 결과와 막힌 지점**입니다. 문제가 없으면 어느 경로와 주장을 확인했는지 기록합니다. Day 2에서 수정할 문제 또는 문제없음의 근거를 찾을 수 있으면 이 단계를 마칩니다.
...
다음은 11주차에서 살펴본 정리 기능의 출력 정확성과 전달 글의 근거를 구분하는 해설입니다. 가상 사례에 수정 후 출력 B가 다음과 같이 제시됐다고 가정합니다. **A/B는 해설을 위해 만든 비교 자료이며 실제 파일·모델 실행 결과가 아닙니다.**
...
자신의 사용 안내에서 대표 경로를 하나 골라 실제 파일·설정·화면과 결과까지 따라갑니다. Skill이라면 호출 예에서 실제로 읽힌 Skill과 출력까지, Workflow라면 가져오기 안내에서 export·필요 연결·대표 실행까지 연결합니다. 글은 그 기능의 실제 사용 위치와 주장에 사용한 원문·실행 결과를 연결해 대조합니다. 공식 문서에 기능이 있다는 설명과 자신의 실행에서 그 기능을 사용했다는 관찰도 구분합니다.
...
Skill 호출이나 Workflow import처럼 터미널이 필요 없는 결과물은 실제 도구의 순서를 적습니다. 전달 글에는 출처 링크와 읽는 순서에 더해 실제 기능을 사용할 위치를 제공합니다. 필요한 계정·의존성·연결과 비용 조건은 재현에 필요한 만큼 안내합니다.
...
기능을 다시 한 번 사용하고 전달 안내를 읽어 현재 목적과 추가 요구에 충분한지 판단합니다. 검토 프롬프트로 도움을 받아도 되지만 최종 수용은 학습자가 결정합니다. 검토가 끝났다는 이유만으로 업로드·공개·배포를 실행하지 않습니다.
...
최종 안내에서 결과·사용법·근거·한계와 실제 전달 상태를 찾을 수 있고, 안내한 대표 경로를 확인했다면 Day 5를 마칩니다. 현재 목적에 충분한 작동 기능과 그 기능을 사용할 전달본이 준비돼야 12주차 전체를 완료합니다. 실제 운영을 주장하지 않는 결과물에 운영 매뉴얼·관측 시스템·배포 실험을 추가하지 않습니다.
...
- [ ] 안내를 따라 11주차 기능의 대표 입력과 추가 요구의 동작을 실제로 확인했습니다. 막힌 부분은 수정·재확인했고, 없었다면 확인한 범위를 남겼습니다.
...
- [ ] 다른 사람이 기능을 사용할 방법이 있습니다. 글·보고서로 전달할 때도 그 기능과 실제 근거로 이어집니다.
