...
### 기능 테스트와 회귀: 동작을 무엇과 비교하나요?

**테스트**는 코드를 정해진 입력으로 실행하고 실제 결과를 기대값과 비교하는 코드입니다. 이번 `refund.py`의 `refund_amount`는 결제액에서 수수료를 빼되 환불액이 0보다 작아지지 않도록 합니다. 예를 들어 결제액 1000원·수수료 2000원의 기대값은 0원입니다. “실행 중 오류가 없었다”는 사실만으로는 이런 계산 규칙을 지켰는지 알 수 없으므로, `tests/test_refund.py`가 실제 반환값을 0과 비교합니다.

이전에 맞던 동작이 변경 뒤 깨지는 것을 **회귀(regression)**라고 합니다. 이 주차는 정상 코드에서 하한 처리 한 줄을 잠시 바꾸어 실패를 만들고, 같은 테스트로 수정 전후를 비교합니다. 기대값은 README의 요구에서 나왔으므로 실패했다고 -1000으로 낮추지 않습니다. 검사 결과가 알려 주는 범위는 작성된 사례까지이며, 실제 결제 서비스 전체의 정확성을 보증하지는 않습니다.

### 종료 코드와 테스트 출력: 실패했다는 사실과 실패 이유

**종료 코드(exit code)**는 명령이 끝나면서 돌려주는 정수입니다. 일반적으로 0은 성공, 0이 아닌 값은 실패를 나타내며 구체적 의미는 명령에 달려 있습니다. 이번 Python `unittest` 명령은 테스트가 통과하면 0, 실패하면 0이 아닌 코드로 끝납니다. **테스트 출력**에는 어느 입력에서 기대값과 실제값이 달랐는지 같은 상세 정보가 나옵니다.

예상 사례로, 환불 하한 처리를 지운 뒤 검사하면 종료 코드는 실패를 알리고 테스트 출력은 “기대값 0, 실제값 -1000”을 보여 줍니다. 종료 코드만 보면 실패 원인까지는 알 수 없습니다. 반대로 테스트 파일을 찾지 못해 시작에 실패한 경우는 환불 계산이 틀렸다는 근거가 되지 않습니다. 코드를 고칠지 실행 위치·환경을 고칠지는 상세 출력을 보고 결정합니다.

### Hook과 이벤트: 특정 시점에 정해진 동작 연결하기

**Hook**은 어떤 사건이 발생했을 때 미리 정한 동작을 실행하도록 연결하는 기능입니다. 연결의 기준이 되는 사건을 **이벤트(event)**라고 부릅니다. 이 주차의 **PostToolUse**는 Codex의 지원되는 도구가 실행 결과를 낸 뒤 발생하는 이벤트입니다. 여기에 Python 스크립트를 연결하면 명령의 종료 코드를 읽어 실패를 알릴 수 있습니다. [공식 Hooks 안내](https://learn.chatgpt.com/docs/hooks#posttooluse)

제공 Hook은 명령 결과를 기록하고 실패 피드백을 반환합니다. 환불 테스트를 직접 실행하거나 계산 코드를 자동 복구하지는 않습니다. 따라서 순서는 **검사를 실행함 → Hook이 그 결과를 읽음 → 실패 원인을 찾아 수정함 → 같은 검사를 다시 실행함**입니다. 앞의 검사를 실행하지 않은 채 Hook만 활성화하면 환불 동작은 확인되지 않습니다.

Codex에 “테스트를 실행해 달라”고 요청하면 Codex의 명령 실행이 이벤트를 만들 수 있습니다. 학습자가 IDE 터미널에 직접 입력한 명령은 Codex 도구 실행을 거치지 않으므로 이 Hook이 관찰하는 대상에 들어가지 않을 수 있습니다. Day 1~2에서는 검사 자체를 먼저 보고, Day 3에서는 같은 검사를 Codex가 실행했을 때 Hook이 연결되는지 확인합니다.

### JSON 설정·matcher·스크립트: 언제 실행하고 무엇을 하나요?

**JSON**은 이름과 값을 대응시켜 설정이나 데이터를 적는 형식입니다. 제공 `.codex/hooks.json`에는 이벤트 이름 `PostToolUse`, 대상을 고르는 `matcher`, 실행할 `command`·`commandWindows`가 있습니다. **matcher**는 이벤트의 도구 이름이 설정한 조건에 맞는지 고릅니다. 이 예제의 값 `Bash`는 Codex Hook이 셸 명령 도구를 부르는 이름이며, Windows에서도 학습자가 Bash 셸을 새로 설치하라는 뜻은 아닙니다. [공식 Hook 도구 범위](https://learn.chatgpt.com/docs/hooks#tool-coverage)

설정은 “언제 어떤 프로그램을 실행할지”를 정합니다. 실제 결과 해석은 `.codex/hooks/post_tool_use_review.py`가 합니다. 스크립트는 전달받은 이벤트 데이터에서 종료 코드를 읽고, 시각·도구 이름·종료 코드·턴 ID(대화에서 해당 실행을 구분하는 값)를 기록합니다. 0이 아닌 명령 종료 코드를 확인했을 때 짧은 피드백을 출력합니다. 이벤트의 필드 이름과 처리 분기는 Day 3에서 실제 코드와 함께 읽습니다.

```text
quality_demo/                           # 이번 작업 폴더
├─ refund.py                            # 환불액 계산
├─ tests/test_refund.py                  # 입력·기대값을 비교하는 테스트
└─ .codex/
   ├─ config.toml                       # 제공 예제의 Hook 기능 설정
   ├─ hooks.json                        # PostToolUse → 대상 도구 → 실행 명령
   └─ hooks/post_tool_use_review.py      # 결과를 읽고 기록·피드백

Codex가 명령 실행 → 테스트 결과와 종료 코드
                     ↓ PostToolUse 이벤트
                 hooks.json의 대상 선택 → 스크립트
                                            ├─→ 실패 피드백
                                            └─→ ../.local/raw/hook-events.jsonl
```

트리는 주요 파일과 연결만 표시합니다. 마지막 로그 경로는 `quality_demo/`를 작업 폴더로 실행한 경우이며, 코드가 작업 폴더의 상위 폴더 아래에 기록하기 때문입니다. `.jsonl`은 JSON 객체를 한 줄씩 이어 쓰는 기록 형식입니다. 설정 파일이 있다는 것과 현재 Codex 표면에서 Hook이 활성화돼 실행됐다는 것은 따로 확인합니다. 신뢰·활성화 절차는 Day 3에서 다룹니다.

### 검사·자동화·실행 권한: 각각 무엇을 확인하나요?

기능 테스트는 환불 계산이 기대 동작을 만족하는지 확인합니다. Hook은 선택한 도구 실행의 결과를 관찰합니다. 실행 권한은 그 명령이나 Hook을 실행해도 되는지 정합니다. 권한을 허용했다고 계산 결과까지 맞아지는 것은 아니며, Hook이 기록을 성공적으로 남겼어도 원래 테스트는 실패한 상태일 수 있습니다.

이 구분은 다른 프로젝트에도 적용됩니다. IDE 테스트 버튼이나 명령은 검사를 직접 시작하는 방법이고, **CI(지속적 통합)**는 저장소 변경 등에 맞춰 검사를 자동 실행하는 환경입니다. 어떤 방식이든 실제 정확성을 확인하는 기준은 검사 내용입니다. 이번에는 프로젝트의 기존 테스트와 대표 Hook을 사용해 실패·원인·복구를 연결하고, Day 5에서 자기 프로젝트에 남길 연결 방식을 고릅니다.
...
LangChain은 모델·도구를 연결하는 기능과 `create_agent` 같은 사용 진입점을 제공합니다. 이 agent의 내부 실행 기반은 LangGraph입니다. 그래프는 **노드**라는 처리 단계와 **간선**이라는 다음 단계의 연결로 작업 흐름을 표현합니다. 모델 단계에서 도구 단계로 갔다가 다시 돌아오는 위 흐름도 그래프로 나타낼 수 있습니다. [LangChain 공식 구조 설명](https://docs.langchain.com/oss/python/langchain/overview#built-on-top-of-langgraph)

이번에는 준비된 agent를 쓰므로 별도의 그래프를 설계하지 않습니다. `config={"recursion_limit": 6}`은 제공 코드가 그래프 실행이 반복되는 데 상한을 두는 설정입니다. 이를 API 요청 정확히 6회 또는 도구 정확히 6회라는 뜻으로 읽으면 안 됩니다. 구체적인 실행 단계와 종료 위치는 코드·메시지에서 확인합니다. [LangGraph 공식 실행 단계 제한](https://docs.langchain.com/oss/python/langgraph/errors/GRAPH_RECURSION_LIMIT)
...
### 목적과 범위 — 누구의 어떤 일을 어디까지 해결할지 정합니다

목적은 결과를 통해 해결하려는 문제이고, 범위는 이번에 만들거나 고칠 부분입니다. 예를 들어 “팀원이 변경 내역을 빠르게 검토하도록 한다”가 목적이라면, 이번 범위를 “입력받은 변경 내역을 릴리스 메모 초안으로 정리한다”로 정할 수 있습니다. 코드 저장소에서 변경을 자동 수집하거나 외부에 게시하는 기능은 같은 목적에 도움이 될 수 있어도 첫 결과의 필수 범위는 아닙니다.

범위를 줄인다는 것은 사용에 필요한 핵심을 남기는 결정입니다. 정리할 변경 내역을 직접 입력받는 방식으로 시작할 수 있지만, 없는 기능을 만들어 쓰는 초안을 그대로 수용하면 검토를 돕는 목적이 흔들립니다. 도구를 덜 사용해도 목적이 충족되는지와 결과가 필요한 사실을 보존하는지를 함께 봅니다.

### 인수 조건 — 결과를 받아들일 기준을 구체화합니다

인수 조건은 결과가 어떤 상태여야 사용할 수 있는지 정한 기준입니다. “릴리스 메모가 자연스럽다”만으로는 사실 오류를 판별하기 어렵습니다. 이 사례에서는 “입력의 변경을 빠뜨리지 않고, 입력에 없는 기능이나 성능 향상을 추가하지 않는다”가 더 구체적인 기준입니다.

입력이 “로그인 오류 수정 / 설정 화면 문구 변경”인데 출력이 “로그인 속도 향상 / 새 설정 기능 추가”라면, 두 항목으로 보기 좋게 나왔어도 기준에 맞지 않습니다. 반대로 같은 사실을 다른 문장으로 정확하게 표현했다면 특정 문구와 일치하지 않는다는 이유만으로 실패로 볼 필요는 없습니다. 인수 조건은 원하는 문장을 외우게 하기보다 사용할 수 있는 결과의 경계를 정합니다.

이 조건의 대상은 본 작업의 결과입니다. “Skill 파일을 작성했다”는 제작 활동이고, “입력에 없는 기능을 결과에 추가하지 않는다”는 릴리스 메모를 받아들일 기준입니다. 지침 파일이 존재한다고 두 번째 조건까지 충족된 것은 아니므로 실제 입력에 사용해 확인합니다. 목적·범위·인수 조건은 별도 계약 문서를 먼저 완성하는 대신 AI가 제안한 초안을 검토하면서 기존 노트에 정리해도 됩니다.

### 첫 결과와 처리 구조 — 입력에서 사용할 결과까지 연결합니다

첫 결과는 목적과 인수 조건을 실제 사례에서 확인할 수 있을 만큼 작게 만든 결과물입니다. 위 작업에서는 변경 내역 한 건을 넣어 릴리스 메모 초안을 받아 보는 흐름이 됩니다. 설정 파일만 만들어 두는 것보다 사용자가 결과를 읽고 판단할 수 있는 지점까지 연결하는 것이 중요합니다.

```text
원자료: 로그인 오류 수정 / 설정 화면 문구 변경
  → 처리 지침: 사실을 보존해 읽기 쉽게 정리
  → 출력 형식: 변경 항목을 글머리표로 표시
  → 결과 확인: 입력의 사실이 보존됐는지 사람이 대조
```

위 구조는 역할을 설명하기 위한 예시이며 각 줄마다 파일이나 도구를 만들라는 뜻은 아닙니다. 간단한 Skill이라면 지침과 형식을 `SKILL.md` 하나에 둘 수 있습니다. 이미 있는 앱이나 Workflow가 적합하면 그 구성을 사용합니다. Day 1에서 만든 결과를 Day 2에 실제로 읽고, Day 3에서는 문제가 생긴 역할에 해당하는 지침·설정·코드를 찾아 고칩니다.

### 확인 근거 — 만들었다는 사실과 사용할 수 있다는 판단을 연결합니다

확인 근거는 인수 조건을 충족하는지 보여 주는 실제 입력·출력·동작 또는 원문입니다. 릴리스 메모가 입력을 보존하는지는 두 내용을 대조해서 확인할 수 있습니다. 그 대조만으로 실제 제품의 로그인 오류까지 수정됐다고 판단할 수는 없습니다. 제품 코드의 동작은 다른 주장이고 별도의 근거가 필요합니다.

같은 원리로 코드 결과물은 대표 기능과 관련 테스트를, 자료를 설명하는 결과물은 주장과 출처를 확인합니다. 결과의 종류에 맞는 근거를 기존 노트에 연결하면 됩니다. 첫 입력에서 발견한 문제를 고친 뒤 같은 입력을 다시 확인하고, Day 4에서는 약속한 사용 범위에서 다른 정상·실패 사례도 살펴봅니다. 사례 수나 도구 수보다 어떤 조건을 확인했는지가 판단의 중심입니다.
...
### 검증과 근거 — 약속한 결과를 실제 자료와 대조합니다

검증은 결과가 약속한 조건을 충족하는지 실제 동작이나 원문으로 확인하는 일입니다. 근거는 그 판단에 사용한 입력·출력·테스트 결과·출처입니다. 설명이 상세하거나 AI가 완료했다고 보고했다는 사실만으로 실제 동작을 확인한 것은 아닙니다.

11주차의 릴리스 메모에서 “입력에 없는 성능 향상을 추가하지 않는다”는 조건을 정했다면, 현재 지침으로 얻은 출력과 같은 입력을 대조합니다. 출력 형식만 검사하면 “오류 수정”을 “속도 향상”으로 바꾸는 의미의 오류를 놓칠 수 있습니다. 형식 검사, 내용 대조, 실제 호출은 서로 확인하는 범위가 다르므로 필요한 근거를 고릅니다.

여기서 확인한 버전도 중요합니다. 수정 전 지침으로 얻은 올바른 출력이 있어도 현재 수정본의 동작까지 증명하지는 않습니다. Day 1에서는 안내에서 가리킨 파일·설정과 실제 결과가 서로 연결되는지 따라가고, Day 2에서는 어긋난 부분을 고쳐 같은 경로로 다시 확인합니다.

### 사용 안내와 재현 — 다른 사람이 결과에 도달할 경로를 제공합니다

사용 안내는 결과를 실행하거나 검토할 순서입니다. 재현은 필요한 조건에서 그 경로를 따라 결과를 다시 확인하는 일입니다. 모델이 문장마다 똑같은 답을 쓰도록 요구하는 것은 아니며, 안내한 기능과 인수 조건을 확인할 수 있는지가 핵심입니다.

Skill이라면 설치하거나 열 위치, 실제 호출 예, 입력과 기대 동작이 필요합니다. Workflow라면 export를 여는 방법과 다시 연결할 모델·자료가 필요하고, 글이나 보고서라면 주요 주장에 도달하는 원문·출처의 경로가 필요합니다. 현재 파일의 이름만 적어 두면 작성자는 알아볼 수 있어도 처음 보는 사람은 어디서 시작할지 모를 수 있습니다.

예를 들어 “아래 메모를 이 Skill에 넣고, 입력에 없는 날짜가 추가됐는지 확인한다”는 안내는 사용과 판단을 연결합니다. 실행에 필요한 경로나 계정이 빠져 중간에 막힌다면 그 부분도 결과물의 사용 문제입니다. Day 3에서는 처음부터 새 매뉴얼을 작성하기보다 실제로 막힌 단계와 근거를 찾기 어려웠던 설명을 기존 안내에 보완합니다.

### 확인 범위와 한계 — 증거가 뒷받침하는 만큼 설명합니다

확인 범위는 어떤 입력·환경·동작을 실제로 살펴봤는지입니다. 한계는 그 확인에서 알 수 없었던 부분이나 아직 해결하지 못한 문제입니다. 입력 한 건이 정확했다면 그 사례의 결과를 확인했다고 할 수 있지만, 어떤 입력에도 정확하다고 넓혀 말할 근거는 아닙니다.

실행했는데 기준과 다르면 실패이고, 필요한 환경에 접속하지 못해 실행하지 않았다면 아직 확인하지 못한 상태입니다. 전달 안내에서는 후자를 `NOT_VERIFIED`로 표시해 다음 사람이 확인할 대상을 알게 합니다. 자신이 지원한다고 약속하지 않은 모든 기능을 추가로 검증할 필요는 없습니다. 실제로 약속한 핵심 조건을 확인할 수 있는지 판단합니다.

### 인수인계와 전달 상태 — 결과와 다음 행동을 함께 넘깁니다

인수인계는 다른 사람이 결과를 사용하거나 작업을 이어 갈 수 있도록 결과물의 위치, 사용 방법, 확인 범위와 남은 일을 연결해 전하는 것입니다. 앞선 사용 안내와 검증 근거가 그 내용이 됩니다. 기존 README나 `handoff.md` 한 곳에서 필요한 위치로 이어지면 충분하며 같은 내용을 여러 보고서에 반복할 필요는 없습니다.

전달본 준비, 공개, 배포는 각각 다른 상태입니다. 비공개로 쓸 Skill과 사용 안내를 준비했다면 전달 준비가 된 것이고, 공개 저장소에 올렸는지는 별도로 확인할 일입니다. 실행 중인 서비스에 변경을 반영하는 배포가 필요 없는 결과물도 있습니다. Day 4에서는 실제 전달할 사본과 독자 범위를 정하고, Day 5에서는 수행한 상태와 수용·보류 이유를 기록합니다. 다른 사람이 첫 행동과 남은 한계를 찾아 사용할 수 있는지가 이번 주의 목표입니다.
