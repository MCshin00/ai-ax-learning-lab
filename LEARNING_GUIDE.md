...
| 6 | 애플리케이션이 모델과 Tool을 어떻게 호출할까? | Tool Calling 앱과 대화 이어받기 |
...
| 11 | 업무 요구를 실제 AI 앱으로 어떻게 연결할까? | 주문 조회·정책 검색·대화와 실패 처리가 연결된 에이전트 |
| 12 | 원하는 포트폴리오를 어떤 범위로 시작할까? | 학습자가 선택한 목적·범위와 첫 사용 가능한 흐름 |
| 13 | 선택한 결과물을 어떻게 완성하고 전달할까? | 목적에 필요한 완성도·실행 안내·데모 또는 전달 자료 |

11주차는 제공 업무로 주문 조회와 정책 검색을 통합하는 AI 애플리케이션을 설계·구현합니다. 7주차와 연결되는 실제 LangChain 기반 Python을 사용합니다.

12~13주차는 원하는 포트폴리오를 구체화하고 제작하도록 돕습니다. 주제·기능·스택·결과물 형태·배포 여부를 고정하지 않으며 기존 프로젝트가 없어도 진행할 수 있습니다. 앞서 배운 모든 기술을 넣는 것이 아니라 선택한 목적에 필요한 구성을 판단합니다.
...
주차의 해설은 조건이 달라지면 어떤 선택을 바꿀지도 설명합니다. 이를 읽고 새로운 요구에서 기술의 사용 위치를 떠올릴 근거를 갖춥니다. 판단은 현재 실습의 설계 대화·코드·실제 결과에서 확인하며, 확인할 근거가 없으면 학습자가 이해했다고 대신 기록하지 않습니다.
...
1~11주차의 제공 업무와 입력은 해당 기술의 책임과 선택을 드러내기 위한 사례입니다. Skill Creator·SDK·프레임워크·AI로 설계안과 구현을 만들고, 필요한 후속 요구도 같은 실습 안에 연결합니다. 12~13주차에는 학습자가 원하는 포트폴리오를 선택합니다. 기존 결과물이 있으면 이어 사용할 수 있고, 없다면 관심과 목적을 구체화하는 대화부터 시작합니다.

### 실습 언어와 설명의 기준

실습의 기본 언어는 Java이며 Gradle Wrapper를 사용합니다. 실제 LangChain·LangGraph 비교가 필요한 7주차와 Dify Tool Plugin을 만드는 9주차에만 Python을 사용합니다. 개념·설계 판단·완료 기준은 입력과 결과, 책임 분리, 상태와 실패 처리 등 다른 언어에도 적용되는 원리로 설명합니다. 실제 코드의 파일·메서드·입출력은 그 원리를 확인할 근거이며, 언어별 문법과 라이브러리 사용법은 필요한 보충 설명입니다.
...
파일 편집·diff·빌드·테스트·일반 프로그램 실행·앱 설정은 **IDE나 앱 화면으로 충분하면 그 경로를 먼저 사용합니다.** Java 실습은 IDE의 Gradle 창이나 메인 클래스 실행 설정을, Python 실습은 선택한 가상환경의 파일·모듈 실행 설정을 사용합니다. 인자와 환경변수는 해당 실행 설정에 넣고 결과는 실행 콘솔에서 확인합니다.

CLI는 필요한 기능이거나 **자동화·프로세스 연결·반복 실행에서 더 적합할 때** 사용합니다. 예를 들어 Inspector를 서버 실행 명령과 함께 시작하거나, 하네스가 Codex와 검사 프로세스를 호출하거나, Plugin 패키지를 만드는 작업에는 CLI를 사용할 이유가 있습니다. Worktree도 앱에서 다룰 수 있으면 앱 기능을 쓰고, 같은 기준 커밋과 경로를 명령으로 정확히 지정할 필요가 있으면 CLI를 선택합니다.

주차의 명령 예시는 해당 경로를 사용할 때의 대안 또는 자동 실행 설정입니다. IDE에서 마친 빌드·검사를 터미널에서 다시 할 필요는 없습니다. 터미널이 필요한 절차에는 Windows PowerShell과 macOS·Linux·WSL 명령을 함께 제공합니다. Gradle 작업을 터미널에서 실행할 때는 Windows의 `.\gradlew.bat`, macOS·Linux·WSL의 `./gradlew`를 사용하며 Gradle을 별도로 설치하지 않습니다. JDK와 빌드 실행 환경은 1주차 안내를 따릅니다. 과정 원본의 생성·동기화 도구 `course.py`는 제작·관리 작업을 반복 적용하는 CLI이며 학습자가 구현할 대상은 아닙니다.

`text` 블록의 자연어 요청은 셸 명령이 아니라 앱이나 IDE 확장의 대화창에 보낼 내용입니다. 코드 블록이라는 이유로 터미널에 붙여 넣지 않습니다.

API 비용과 외부 변경이 있는 실습은 사용할 계정·대상·범위를 확인합니다. 계정이나 기능이 없어 실행하지 못했다면 `NOT_VERIFIED`와 이유를 적고 해당 단계로 돌아올 수 있게 합니다. 오프라인 예제 성공은 실제 서비스 호출 성공을 뜻하지 않습니다. 실제 연결이 없어도 로컬 제작과 다음 학습은 진행할 수 있지만, 실제 모델·Host 사용을 확인해야 하는 완료 조건은 남겨 둡니다. 주차 이동은 자료 준비이며 역량의 자동 완료가 아닙니다. 12~13주차의 포트폴리오는 학습자가 선택한 목적과 범위로 완료 기준을 정합니다.
...
프롬프트는 내용을 읽고 앱이나 IDE 확장의 대화창에서 직접 보냅니다. 터미널 중심의 작업 환경을 선택한 경우에는 대화형 CLI를 사용할 수 있습니다. 제공 문구를 그대로 쓰거나 자신의 요청에 맞게 고쳐도 됩니다. 도구가 초안을 만들었다면 필요한 부분만 읽고 실행 결과와 대조합니다. 파일이 많다는 이유로 모두 분석할 필요는 없습니다.
...
수용 방법도 이 요구에서 나옵니다. `"  결제   오류\n문의  "`의 기대 결과와 빈 입력의 예외, 이모지의 길이 경계는 서로 다른 오해를 드러냅니다. 기존 테스트를 사용하고 변경된 함수와 기대값의 diff를 읽으면 됩니다. 이 구성에서 README는 업무 계약, B 요청은 이번 실행의 순서, AGENTS.md는 반복되는 작업 원칙, 테스트는 실제 입력으로 수용 판단을 돕는 역할을 맡습니다.

아래 A/B를 시작하기 전에 AI에게 세 구성의 차이를 위 입력에 적용해 설명하게 하고, B에서 사용할 순서와 수용 근거에 대한 의견을 짧게 보냅니다. 합의한 요청을 아래 실습에 사용합니다.
...
요구가 바뀌었으므로 기존의 “81개를 80개로 잘라 반환” 기대값도 선택한 계약에 맞게 바뀝니다. 공백 처리·빈 입력처럼 계속 필요한 기준은 유지하되, 오류나 결과 객체 중 선택한 방식으로 확인합니다. 기존 테스트와 필요한 호출 예에서 선택한 계약의 차이를 확인합니다.
...
필요한 실행을 하지 못한 부분은 이유와 함께 미확인으로 남깁니다.
...
**남길 결과·완료 판단:** 두 구성에서 선택·처리·표현의 역할을 원문과 연결하고, 현재 구성을 유지하거나 나눌 이유와 변경 시 함께 살펴볼 위치를 대화에서 검토합니다. 그 관계를 기존 노트에 연결하면 Day 2를 마칩니다. 제공 예제의 추가 실행과 파일 분리는 선택이며, 제작은 Day 4의 요구로 이어갑니다.
...
AI에게 현재 Skill의 적용 설명·처리 규칙·출력 형식을 결과의 해당 부분에 짚어 설명하게 합니다. 다음에 맡길 업무가 달라지면 적용 범위를, 묶음 기준이 달라지면 처리 규칙을, 결과 배치가 달라지면 출력 형식을 살펴볼 수 있습니다. 고객 의견이나 회의 의견 정리에도 출처와 관찰을 나누는 판단은 이어 쓸 수 있지만, 공통 목적과 필요한 자료가 같은지는 그 업무에서 다시 판단합니다. 이번 결과의 의미를 기존 `skill-analysis.md`에 이어 연결합니다.

**남길 결과·완료 판단:** 새 제보 묶음과 기존 단일·범위 밖 요청의 실제 결과를 확인하고, 적용 범위·처리 규칙·출력 형식이 그 차이에 어떻게 연결됐는지 대화에서 해석합니다. 기존 노트의 결과에 재사용한 기준과 다음 변경에서 살펴볼 위치를 연결하면 Day 5를 마칩니다.
...
## 모델이 사용하는 앱으로 이어지는 연결

이 주차의 클라이언트는 호출할 도구와 인수를 코드에서 정합니다. Codex Host에서는 모델이 자연어를 해석해 도구를 고릅니다. 6주차 Day 5에서는 같은 상품 서버를 자신이 만든 LLM 앱에 연결해 이 선택·실행·반환을 구성합니다. 이번에 만든 서버와 도구 계약을 재사용합니다.
...
후속 요구에서 정했던 범위·반환 약속·실행 순서가 실제로 충분했는지 돌아봅니다. 긴급 조건처럼 두 정보를 함께 써야 하는 부분을 누구에게 언제 맡겼고, 그 선택이 통합에 충분했는지 실제 결과와 대조합니다. 이 근거로 다음 작업에서 병렬로 맡길 부분과 순차로 처리할 부분을 설명합니다.
...
이 주차에서 두 Worktree를 사용한 이유는 파일 격리와 변경 통합을 실제로 관찰하기 위해서입니다. 같은 크기의 모든 작업을 둘로 나누라는 권고는 아닙니다. 분할 여부는 독립적으로 진행할 수 있는 일과 인계·통합에 필요한 조정을 함께 보고 정합니다. 요구의 규모가 커지더라도 독립성과 조정 비용을 기준으로 분할 여부를 판단합니다.
...
**남길 결과·완료 판단:** 실제 업무 결과와 그 운영에 필요한 하네스 구성을 연결합니다. 어떤 자료·규칙·도구·검사가 왜 필요하고 실행 엔진은 그중 어떤 연결을 맡는지 현재 파일과 결과로 설명합니다. 분석은 기존 주차 노트에 이어 씁니다.
...
Windows의 Gradle 소켓 문제가 발생하면 5주차 실행 프로젝트의 `quality_demo/windows-gradle.md`를 엽니다. 기존 프로젝트 로컬 `.codex/config.toml`의 `JAVA_TOOL_OPTIONS`를 보존하고 일반 `TEMP`·`TMP`나 전역 설정을 바꾸지 않습니다.
...
MCP는 다른 프로세스의 기능을 발견·호출하는 계약이고, 이번 Tool Calling은 모델의 요청을 앱의 실행으로 연결하는 계약입니다. 앱의 Tool 실행기가 로컬 `lookup` 대신 3주차 MCP Client를 호출할 수 있습니다. 그때 MCP 서버의 오류를 모델에게 전달할 의미로 변환하는 책임은 앱에 남습니다. Day 5에서 기존 상품 서버의 도구 하나를 이 앱의 모델에 연결합니다. 5주차 엔진이 Codex 프로세스의 실행·검사를 관리했다면, 이번 루프는 그 실행기 안에서 모델과 함수가 왕복하는 원리를 보여 줍니다.
...
## 모델을 앱에 연결하기 전에 구분할 개념

### 학습과 추론

학습은 자료로 모델의 매개변수를 바꾸는 과정이고, 추론은 학습된 모델에 현재 입력을 주어 결과를 얻는 과정입니다. `CustomerDirectory`의 요금제를 바꾸고 다시 조회해 안내하면 **모델을 재학습한 것이 아니라 현재 업무 사실을 입력으로 공급한 것**입니다. 최신 계정 상태의 기준은 모델의 기억이 아니라 조회 결과입니다.

### 맥락 한도와 출력의 불확실성

모델 입력에는 질문, 지침, 도구 정의와 이전 호출 결과도 포함됩니다. 입력이 길어지면 사용할 수 있는 맥락 한도와 출력 여유를 함께 고려해야 합니다. `maxOutputTokens(700)`은 출력 상한이며 긴 입력을 자동으로 요약하는 설정이 아닙니다. 7주차에서는 보관 상태와 모델에 전달할 내용을 실제로 나눕니다.

모델은 입력에 맞는 출력을 확률적으로 생성합니다. 같은 질문에서도 문장이나 도구 선택이 달라질 수 있으므로 한 번 성공한 답변이 모든 표현에서 맞는다는 근거는 아닙니다. 낮은 무작위성 설정이나 올바른 JSON 형식도 업무 사실의 정확성을 보장하지 않습니다.

### 최종 구조화 출력

도구 스키마는 **실행할 함수의 인수**, 최종 응답 스키마는 **다음 코드가 읽을 결과**의 계약입니다. `StructuredAnswer.format()`은 Responses API의 `text.format`에 `json_schema`와 `strict`를 지정합니다. 프롬프트에서 JSON을 요청하는 것과 SDK에 스키마를 지정하는 것을 구분합니다. [공식 구조화 출력 안내](https://developers.openai.com/api/docs/guides/structured-outputs)

`customer_id`, `plan`, `account_status`, `needs_follow_up`, `answer`는 이 고객 안내 사례에서 고른 필드입니다. 조회에 성공했는데 모델이 다른 요금제를 반환하면 스키마에는 맞아도 `FACT_MISMATCH`로 보류합니다. `answer`의 자유 문장까지 이 필드 검사로 증명되지는 않으므로 원문과 대조합니다. 거절·불완전 응답·통신 실패는 파싱 가능한 정상 계정 결과로 바꾸지 않습니다.

실행 자료의 `StructuredAnswer.java`는 조회 → 생성 → 검사의 짧은 참고 경로이고, `McpAssistant.java`는 기존 호출 루프에 MCP 실행기를 연결합니다. IDE의 Gradle 동기화로 `build.gradle`에 선언된 MCP SDK 1.1.4도 준비합니다. 자기 앱을 계속 사용할 때는 필요한 의존성과 연결부를 그 앱에 반영하며 전체 구조를 참고 클래스에 맞추지 않습니다.
...
### 실제 인자와 다음 요청을 관찰하는 방법

이 관찰에서는 **모델이 요청한 값, 앱이 실행한 함수의 반환값, 다음 모델 요청에 담긴 결과가 연결되는지** 확인합니다. 기본 콘솔의 `tool_results`에는 호출 ID·이름·결과가 있지만 `arguments`와 다음 요청의 전체 입력은 나오지 않습니다. 위 정상·없는 고객 실행을 IDE의 **Debug**로 진행하고, `src/main/java/lab/week06/Quickstart.java`의 `run`에서 다음 위치를 살펴봅니다.

1. `var toolResult = executeCall(call.name(), call.arguments());`에 중단점을 둡니다. 실행 전에 `call.name()`, `call.arguments()`, `call.callId()`를 읽고 `TOOL`에 선언한 입력과 대조합니다. 제공 대역의 정상 인자는 `customer_id=C-100`, 없는 고객은 `C-404`이며 첫 호출 ID는 `offline-1`입니다.
2. 그 줄을 한 단계 실행한 뒤 `toolResult`를 봅니다. 정상 입력에서는 제공 고객의 `plan=basic`과 `status=active`가, 없는 고객에서는 `error=CUSTOMER_NOT_FOUND`가 반환됩니다. 호출을 요청한 모델과 고객 데이터를 조회한 함수가 맡은 역할을 이 두 값의 차이로 구별합니다.
3. 다음 반복의 `client.create(request.build())`에서 멈춥니다. 중단점 조건을 `index == 2`로 두면 두 번째 요청을 볼 수 있습니다. IDE의 식 평가에서 `request.build().input().orElseThrow().asResponse()`를 열어 `function_call` 항목의 인자·호출 ID와 `function_call_output` 항목의 호출 ID·`output`을 대조합니다. 두 호출 ID가 같고, `output`에 방금 확인한 함수 결과가 들어 있어야 합니다. 앞선 응답 항목도 함께 전달되는지 확인한 뒤 실행을 이어 최종 안내와 비교합니다.

기존 검사로 연결을 확인할 때는 `src/test/java/lab/week06/QuickstartTest.java`의 `actualLookupResultReturnsWithMatchingCallId`와 `unknownCustomerReturnsAnErrorToTheModel`을 사용합니다. 첫 검사는 고객 요금제를 잠시 `changed-plan`으로 바꿔 실제 조회값이 답변에 반영되고 두 번째 요청의 호출·결과 ID가 같은지 확인합니다. 둘째 검사는 없는 고객의 오류가 다음 모델 응답에 반영되는지 확인합니다. `reasoningItemsArePreservedBeforeToolResult`는 결과 앞에 원래 추론 항목을 보존하는 연결을 검사합니다. 이 대역 실행으로 확인하는 것은 앱의 값 전달이며, 실제 모델의 도구 선택과 문장 생성은 실제 연결 결과에서 판단합니다.

```text
정상 고객과 없는 고객을 비교하겠습니다. 현재 코드에서 실제 arguments, 함수 반환값,
다음 요청의 호출·결과 대응 정보를 볼 중단점과 변수 또는 기존 테스트를 안내하세요.
내가 확인한 값이나 기존 실행 근거를 Tool 정의와 대조해 값이 전달된 경로를 설명하세요.
업무 오류도 모델에 전달하는 이유와 전송 실패로 응답을 받지 못한 경우의 차이를 짚어주세요.
코드에서 예상한 값과 실행 중 확인한 값을 구분하고 결과 해석은 api-note.md에 이어 주세요.
```

**남길 결과·완료 판단:** 하나의 입력에서 도구 선택→인자→실행값→다음 응답까지 추적하고, 조회 실패와 모델 응답 완료의 차이를 설명합니다. 정상·없는 고객의 관찰값과 확인한 변수 또는 기존 테스트 위치를 노트에 연결합니다. 대역에서 확인한 다음 요청과 실제 모델에서 받은 결과의 범위를 구분합니다. 여기서는 `plan`과 `status`를 함께 반환하는 연결을 확인하고, Day 3에서 이 연결의 공개 계약과 결과 처리를 수정합니다.
...
함수 호출의 실제 인자, 앱의 반환값, 다음 모델 요청에 담긴 호출·결과를 관찰할 중단점과 변수 위치를 안내하세요.
선택한 Gemini API의 이름·호출 식별자·응답 보존 규칙으로 대응을 확인하고 OpenAI의 call_id 필드명을 강제하지 마세요.
...
고객 안내를 화면에 표시하거나 고객 번호 보완 단계로 넘긴다는 요구를 적용합니다. AI에게 어떤 결과 필드가 필요한지 추천과 이유를 받고 짧게 수용·수정 의견을 냅니다. 기존 조회 함수를 재사용해 스키마 응답을 받는 경계와 그 결과를 소비하는 코드를 연결합니다.

참고 구현은 IDE에서 `StructuredAnswer.main`을 실행합니다. 기본값은 가짜 모델이며 `--live --customer C-100`은 실제 모델, `--live --customer C-404`는 없는 고객 사례입니다. 키와 모델 설정은 앞서 준비한 IDE의 비공유 실행 설정을 사용합니다. 참고 코드의 `consume`에서 조회 사실과 결과 필드를 대조하고 `SHOW_ACCOUNT`, `ASK_CUSTOMER_ID`, `FACT_MISMATCH`로 갈라지는 위치를 읽습니다.

```text
현재 고객 조회 앱에서 최종 결과를 다음 코드가 읽을 수 있도록 구성해주세요.
필요한 필드와 스키마 지정 위치, 사실 대조와 다음 처리의 책임을 추천하고 이유를 설명해주세요.
내 의견을 반영한 뒤 기존 조회 코드를 사용해 구현하고, 실제 모델 실행 방법을 안내해주세요.
정상 고객·없는 고객과 형식은 맞지만 요금제가 틀린 대역을 구분해 확인해주세요.
도구 인자 스키마와 최종 응답 스키마의 차이를 실제 요청 코드로 설명해주세요.
```

**남길 결과·완료 판단:** 실제 스키마 응답과 다음 처리 결과, 잘못된 사실을 보류하는 코드와 판단 이유를 기존 노트에 남깁니다. 참고 대역의 정상 응답만으로 실제 모델의 구조화 출력을 확인했다고 하지 않습니다.
...
### 구조화 결과의 실패도 같은 원칙으로 읽기

`StructuredAnswerTest`의 거절·불완전 JSON·다른 요금제 사례를 봅니다. 거절은 정상 JSON 생성을 약속한 경로 밖일 수 있고, 출력이 끊긴 경우에는 일부 필드만 저장하면 안 됩니다. SDK의 형식 계약과 업무 검사가 각각 어디까지 확인하는지 결과로 설명합니다. 일시적인 전송 오류를 재시도할 경우에는 횟수·대기·전체 호출 예산을 정하고, 잘못된 인증·스키마·업무 입력은 같은 요청을 반복해서 해결하지 않습니다. 이번 참고 구현은 자동 재시도를 끄고 실패를 반환합니다.
...
### 기존 MCP 서버를 LLM 앱에서 사용하기

제공 요청은 “NOTE-01의 가격과 재고를 알려 주세요”입니다. 상품 서버는 3주차 것을 재사용합니다. MCP의 도구 이름·설명·입력 스키마를 모델의 도구 계약으로 전달하고, 모델이 선택한 호출을 `tools/call`로 실행한 뒤 해당 호출 ID에 결과를 돌려줍니다. `McpAssistant.Catalog.tools`가 계약 변환, `execute`가 SDK 호출, `Quickstart.runWithTools`가 반복과 종료를 맡습니다. 모델이 상품 정보를 기억해서 답한 것과 실제 서버 결과로 답한 것을 호출 기록으로 구별합니다.

먼저 3주차 프로젝트의 IDE Gradle 창에서 `installDist`를 실행합니다. 6주차 프로젝트에서 `McpAssistant.main`을 선택하고 프로그램 인수를 다음과 같이 지정합니다. 작업 폴더는 `llm_lab`이며 경로 구분자 `/`는 두 OS에서 사용할 수 있습니다.

```text
--server-lib ../../week03-mcp-integration/learning_lab_server/build/install/learning-catalog/lib
```

이 기본 실행은 **모델만 대역이고 MCP 서버·클라이언트는 실제**입니다. 이어 `--live --text "NOTE-01의 가격과 재고를 알려 주세요."`를 추가해 실제 모델의 선택과 답변을 확인합니다. 다른 서버 진입점을 만들었다면 `--server-main`에 그 클래스 이름을 지정합니다. 같은 도구를 다른 이름·인자로 설계했다면 어댑터의 공개 대상과 인자 검사를 그 계약에 맞춥니다. 고정된 `get_product`가 MCP의 필수 이름은 아닙니다.

```text
기존 상품 MCP 서버의 도구 하나를 현재 LLM 앱에서 사용하도록 연결해주세요.
서버의 이름·설명·스키마를 모델에 전달하는 방법과 호출 결과·오류를 돌려주는 책임을 추천해주세요.
내 의견을 반영해 기존 서버와 호출 루프를 재사용하고 실제 연결 순서를 안내해주세요.
정상 상품, 없는 상품, 서버 연결 실패를 구분해서 확인해주세요.
직접 작성한 서버 진입점이나 도구 계약이 있으면 그것을 유지하고 필요한 어댑터만 맞춰주세요.
```

**남길 결과·완료 판단:** 실제 모델 → MCP → 상품 결과 → 답변의 경로와 성공·도구 오류·연결 실패를 구분한 결과입니다. 3주차의 서버 제작이나 저장·편집 기능을 반복하지 않습니다.

## 완료 기준

- 최종 응답 스키마를 실제 모델에 적용하고 결과를 다음 코드에서 사용했으며, 형식·사실·거절·불완전 응답을 구별했습니다.
- 기존 MCP 도구 하나를 직접 만든 LLM 앱에 연결하고 실제 모델의 선택과 서버 결과 전달을 확인했습니다.
...
체크포인트는 실행을 이어갈 상태를 보관합니다. 그 상태를 모두 다음 모델 입력으로 보내야 한다는 뜻은 아닙니다. 배송 문의가 `O-100`에서 `O-200`으로 정정되었을 때 기록에는 이전 조회가 남을 수 있지만 현재 안내에는 새 주문의 사실이 필요합니다.

참고 `ShippingAgent`의 기본 정책은 이전 메시지를 교체합니다. `retain_history=True`를 선택하면 기록을 보관하고 `ShippingPolicy.wrap_model_call`에서 **현재 업무 입력 이후의 메시지만** `request.override(messages=...)`로 전달합니다. 현재 업무 입력에는 상태에서 보존한 `issue`와 최신 `order_ids`가 다시 들어 있습니다. 이전 대화를 그냥 마지막 한 문장만 남기는 방식과 다릅니다.

모델의 호출 요청과 그 결과 메시지는 함께 유지해야 합니다. 임의의 메시지 개수로 자르면 결과 없는 호출이나 호출 없는 결과가 남을 수 있습니다. 이 사례는 현재 업무 단위로 선택하므로 그 안의 호출·결과 쌍을 함께 전달합니다. 단, 한 업무 자체가 매우 길거나 입력 원문이 크면 이 정책만으로 토큰 상한을 보장할 수 없습니다. 그때는 입력 제한·관련 원문 선택·요약을 추가로 판단합니다. 요약은 표현과 조건을 잃을 수 있어 원문·최신 업무 사실과 역할을 나눕니다. [LangChain의 단기 메모리와 맥락 관리](https://docs.langchain.com/oss/python/langchain/short-term-memory)
...
제공 요구는 “이전 배송 문의 기록을 보관하되 번호 정정 뒤에는 현재 주문만 안내한다”입니다. 기존 상태·재개 기능에 이 요구를 적용합니다. 원래 문의를 어디에 유지하고, 과거 조회를 언제 무효화하며, 다음 모델 입력에서 무엇을 제외할지 AI의 추천을 검토합니다.

IDE에서 `examples/context_window.py`를 기본 실행하면 실제 프레임워크와 가짜 모델로 보관 메시지·모델 입력·결과를 나란히 볼 수 있습니다. `--live`는 실제 모델 연결입니다. `O-100` 기록이 보관되어 있어도 정정 후 모델 입력에는 원래 문의와 `O-200`의 호출·결과가 있어야 합니다. 다른 대화의 입력도 넣어 분리 여부를 확인합니다. 참고 코드 대신 이미 선택한 요약·선별 정책을 적용해 같은 요구를 확인해도 됩니다.

```text
현재 배송 문의 앱에서 보관 상태와 모델에 전달할 맥락을 구분해주세요.
원래 문의·현재 주문은 유지하고 이전 주문의 사실은 현재 답변에 쓰지 않는 정책을 추천해주세요.
내 의견 뒤 구현하고 번호 보충·정정 전후의 실제 모델 입력과 결과를 보여주세요.
도구 호출과 결과의 연결이 유지되는지, 다른 대화가 섞이지 않는지 확인해주세요.
선택한 정책이 토큰 상한·재시작 후 복원까지 보장하는지는 별도로 설명해주세요.
```

**남길 결과·완료 판단:** 저장된 상태와 실제 전달한 맥락의 차이, 원래 문의·현재 사실이 유지된 결과와 선택 이유입니다. 중단·재개 전체를 새로 제작하지 않습니다.

## 완료 기준

- 대화 기록의 보관과 모델 입력의 선택을 구분하고, 현재 문의·도구 호출 쌍을 유지하면서 오래된 사실을 제외하는 정책을 실제 호출로 확인했습니다.
...
접근 제어는 검색 점수를 계산하기 전에 적용합니다. `tenant-alpha`의 요청에서는 `public`과 `tenant-alpha`의 허용 문서를 후보로 고르고, `tenant-beta`의 문서는 제외합니다. 이렇게 걸러진 자료만 검색과 생성에 사용하므로 다른 테넌트의 문서가 모델 입력에 섞이는 것을 막습니다.
...
다음 그림은 제공된 `knowledge_base/`와 `rag_lab/`을 연결한 흐름입니다. 상자는 처리 책임과 주고받는 자료를 나타냅니다. 자료 준비, 검색, 답변 생성을 나누어 보면 어떤 선택이 다음 단계의 입력을 바꾸는지 따라갈 수 있습니다.

### 자료 준비 — 의미 검색에 쓸 인덱스 만들기

```mermaid
flowchart TD
    documents["정책 원문"] --> chunks["문단을 묶어<br/>문서 조각 만들기"]
    metadata["관리 메타데이터<br/>출처·버전·상태·접근·신뢰"] --> chunks
    chunks --> allowed["요청 테넌트에 허용된<br/>현재 문서 조각 선택"]
    allowed --> embed["문서 조각 임베딩"]
    embed --> store[("벡터 저장소<br/>벡터·본문·조각 ID")]
    allowed --> catalog["원문 정보가 붙은 조각 목록"]
    store --> snapshot[("인덱스 파일<br/>저장소·조각 목록<br/>모델·테넌트")]
    catalog --> snapshot
```

원문과 `knowledge_base/manifest.json`을 함께 읽어 조각에 원문 정보를 붙입니다. 공개 자료와 요청 테넌트의 자료 중 상태·신뢰 조건을 통과한 현재 조각을 임베딩하고, 저장소와 조각 목록을 `.local/semantic-index.json`에 함께 저장합니다. 검색에서 받은 조각 ID로 본문과 출처를 다시 찾기 위해서입니다.

의미 검색을 시작할 때는 현재 허용 조각과 저장한 조각, 모델, 테넌트가 일치하는지 확인합니다. 문서 개정 등으로 달라졌으면 재생성이 필요하다는 오류를 반환하며, 인덱스 준비를 다시 실행하면 새 내용으로 교체됩니다. 이 분리 덕분에 질문마다 문서 임베딩을 반복하는 대신 저장한 벡터를 재사용합니다.

### 질문 처리 — 두 검색 경로에서 후보 모으기

```mermaid
flowchart TD
    query["질문"] --> mode{"선택한 검색 방식"}
    mode -->|어휘 검색| lexical["토큰·문자 조각의<br/>어휘 유사도 계산"]
    fresh["실행 시 읽고 나눈 조각<br/>접근·상태 조건 적용"] --> lexical
    mode -->|의미 검색| embed["질문 임베딩"]
    embed --> semantic["질문 벡터와 가까운<br/>문서 조각 검색"]
    index[("확인한 인덱스의<br/>벡터 저장소와 조각 목록")] --> semantic
    lexical --> candidates["임계값을 통과한 검색 후보<br/>조각과 검색 점수 · 최대 8개"]
    semantic --> candidates
    candidates --> ranked["정책 상태·어휘 일치로<br/>재정렬 · 최대 4개"]
```

어휘 검색은 실행 시 읽은 허용 조각과 질문의 표현을 비교합니다. 의미 검색은 준비한 저장소에 질문 벡터를 보내고, 찾은 조각 ID에 해당하는 원문 정보를 후보에 붙입니다. 두 경로에서 얻은 후보는 같은 재정렬과 근거 선택으로 이어집니다. 그림의 8개와 4개는 제공 실행 진입점의 상한이며, 임계값을 통과한 후보가 적으면 실제 개수도 줄어듭니다.

### 근거 선택과 생성 — 후보를 답변의 근거로 연결하기

```mermaid
flowchart TD
    ranked["재정렬한 조각 · 최대 4개"] --> check{"근거가 있고<br/>표시된 충돌이 없는가?"}
    check -->|근거 없음·표시된 충돌| early["생성 전에 답변 보류"]
    check -->|통과| sources["최대 두 문서 선택<br/>문서별 후보 조각 묶기"]
    sources --> mode{"실행 목적"}
    mode -->|검색 결과 확인| evidence["선택한 근거 반환"]
    mode -->|답변 생성| generation["질문 + 출처 ID별 근거 본문<br/>→ 생성 모델"]
    generation -->|응답 도착| validate["응답 상태·답변 형식<br/>출처 ID 검사"]
    generation -->|호출 실패| failure["생성 호출 오류 반환"]
    validate -->|유효한 답변과 출처| answer["답변 + 출처 ID 반환"]
    validate -->|모델이 보류| abstain["답변 보류"]
    validate -->|형식 또는 출처 오류| invalid["출력 오류 반환"]
```

근거 선택은 재정렬에 남은 조각을 문서별로 묶습니다. 예를 들어 환불 문서의 접수 기한과 소유권 확인 조각이 함께 남았다면 같은 출처 아래 두 조각을 전달합니다. `conflicts_with`로 연결된 문서가 후보에 함께 있거나 후보가 비었으면 생성 전에 보류합니다. 근거가 준비되면 검색 결과만 확인하거나, 질문과 근거를 모델에 전달해 답변을 생성합니다.

평가는 이 흐름의 서로 다른 위치를 봅니다. 골든 질문은 질문부터 실행해 기대 문서와 검색 후보를 비교하고, 최종 답변을 실제 전달한 근거·원문 조건과 대조합니다. Red Team 자료는 합성 `질문 + 근거`를 생성 입력에 직접 넣어 자료 속 지시나 근거 없는 요구가 답변에 반영되는지 확인합니다. 따라서 검색에서 빠진 근거, 근거 선택에서 잃은 조건, 생성 과정에서 바뀐 사실을 각각 찾아 수정할 수 있습니다.
...
정확한 제품명·조항 번호로 찾는 상황에서는 어휘 검색이 유용합니다. 일상 표현과 문서 용어가 다른 상황에서는 학습된 임베딩을 검토합니다. 제공 `Retrieval`은 토큰·문자 조각으로 만든 간단한 기준선입니다. Day 3에서는 실제 임베딩 SDK·저장소와 함께 `Bm25Index`의 Lucene BM25, `HybridSearch`의 순위 결합도 연결해 비교합니다. 기준선과 BM25를 같은 구현으로 설명하지 않습니다.
...
자기 앱의 결과는 예를 들어 `Answer(status, text, evidence)`라는 타입일 수 있습니다. 앱의 결과 계약을 정한 뒤, 평가에 전달할 때 `retrieved_document_ids`, `sources`, `model_called`, `status`, `answer`, `source_ids`로 대응합니다. 이 필드들은 후보·실제 근거·생성 여부를 구분하려는 **관찰 형식**입니다. 검색 점수나 출처 ID 소속 검사는 원문이 답변의 주장을 지지하는지까지 판정하지 않습니다.
...
생성 입력에는 원 질문과 선택한 실제 근거를 보냅니다. 이 입력에서 예상하는 결과는 “소유권 확인 전에는 처리를 중단하고 담당자에게 이관한다”는 안내와 출처입니다. 필요한 조건을 찾지 못하면 보류하고, 호출에 실패하면 호출 실패로 돌려줍니다. 이 상태 구분이 있어야 검색을 다시 봐야 할 상황과 호출 환경을 확인할 상황을 분리할 수 있습니다. 평가에서는 같은 질문 처리 진입점을 호출해 후보·근거·답변을 대조합니다.
...
첫 행은 Day 4에서 실제로 적용합니다. 나머지 행은 조건이 바뀌면 선택이 달라지는 이유를 이해하는 해설입니다. Day 3에서 고른 구성과 비교해 같은 책임을 계속 사용할 부분과 바꿀 부분을 읽습니다.
...
BM25는 질문의 단어가 문서에 얼마나 나타나는지, 여러 문서에 흔한 단어인지, 문서 길이가 어떤지를 반영하는 검색 순위 방식입니다. 반복 횟수를 그대로 무한 가산하지 않으며 벡터 임베딩을 호출하지 않습니다. `Bm25Index`는 Lucene의 `BM25Similarity`를 사용하고 `KoreanAnalyzer`로 한국어 단어를 분석합니다. 띄어쓰기·활용형 처리도 검색 결과에 영향을 줍니다. [Lucene BM25](https://lucene.apache.org/core/9_12_3/core/org/apache/lucene/search/similarities/BM25Similarity.html)

### 하이브리드와 RRF

같은 조각을 BM25와 의미 검색으로 찾고 두 후보 목록을 결합합니다. 참고 `HybridSearch.fuse`는 조각 ID별로 각 순위의 `1 / (60 + 순위)`를 더하는 RRF를 사용합니다. 두 검색의 점수 척도가 달라 BM25의 8점과 벡터 유사도 0.8을 그대로 더하지 않습니다. `60`과 두 목록의 동등한 비중은 참고 선택이며 정확도를 보장하는 값이 아닙니다.

두 경로는 **같은 문서·조각·접근 범위**를 사용해야 합니다. 합친 후보를 나중에 거르는 것만으로 비공개 문서가 검색 결과를 좌우한 문제까지 없어지지는 않습니다. 재색인 때 두 색인의 자료 버전을 함께 맞추고, 순위만 달라졌는지 최종 답변에 필요한 조건이 실제로 남았는지 구별합니다.

### 실패에서 바꿀 위치 고르기

| 관찰 | 바꿀 후보와 이유 | 확인할 결과 |
|---|---|---|
| 제품명·정책명 같은 정확한 단어가 후보에서 빠짐 | BM25, 분석기·동의어, 제목·본문의 색인 범위 | 이름이 같은 다른 자료를 혼동하지 않고 필요한 원문을 찾는가 |
| “두 번 빠져나갔어요”를 정책의 “이중 결제”와 연결하지 못함 | 의미 검색 또는 검색어 재작성 | 재작성 전후의 질문 의도와 날짜·대상 조건이 유지되는가 |
| 필요한 조건이 서로 다른 조각에 있음 | 청킹·인접 조각 포함·문맥 선택 | 기한만 남고 예외·승인 조건이 사라지지 않는가 |
| 필요한 후보는 있는데 최종 문맥에서 밀림 | 후보 수·재정렬·MMR | 관련성과 중복 감소를 함께 확인하며 필요한 조건이 빠지지 않는가 |
| 답에 필요한 자료 자체가 없음 | 추가 질문·근거 부족 안내, 요구상 허용된 다른 자료원 | 재검색 횟수만 늘리지 않고 모르는 내용을 구분하는가 |

**질의 재작성**은 검색에 맞게 표현을 바꾸는 것이고 **질의 분해**는 여러 질문으로 나누어 각각 검색하고 결과를 연결하는 것입니다. 분해한 문장을 저장하기만 하면 별도 검색·통합을 구현한 것이 아닙니다. **재정렬**은 이미 찾은 후보의 순서를 다시 정하며, **MMR**은 관련성과 후보 간 중복을 함께 고려합니다. 모델 재정렬은 관련도를 판단하는 추가 호출·비용을 수반할 수 있습니다. 어느 것도 찾지 못한 원문을 자동으로 만들어 주지는 않습니다. 모든 방법을 함께 구현하는 과제가 아니라 실제 실패의 위치에 맞춰 선택할 수 있게 배우는 구분입니다.
...
IDE의 `evaluate` 작업 또는 `.\gradlew.bat evaluate` / `sh ./gradlew evaluate`로 네 질문의 기준선 결과를 읽습니다. 제공 실행기가 나란히 출력한 결과와 검토 기준을 대조합니다.
...
**할 일:** 위 제공 요구에서 자료의 포함 범위·조각화·검색·문맥 선택·색인 수명·결과 계약을 AI와 도출하고 실제 Java 앱으로 구성합니다. 원문과 SDK 저장소·임베딩 호출·생성의 통신 코드는 재사용합니다. `Pipeline → Rerank → Answering`의 각 책임을 자신의 문맥 정책에 맞게 연결합니다. 재정렬을 포함할지는 Day 2에서 확인한 순위 조정의 효과를 근거로 선택합니다.
...
### 같은 자료에 BM25와 하이브리드 연결하기

`Bm25Index.java`, `HybridSearch.java`, `SearchComparison.java`를 읽습니다. Gradle 의존성은 Lucene `core`, `analysis-common`, `analysis-nori` **9.12.3**입니다. 기존 앱에 적용할 때는 해당 의존성을 추가하고 IDE에서 Gradle을 동기화합니다. 별도 검색 서버는 필요하지 않습니다.

IDE에서 `SearchComparison.main`을 기본 실행하면 실제 BM25만 실행합니다. 앞에서 준비한 의미 색인을 사용하려면 프로그램 인수 `--live`를 추가합니다. 같은 `EmbeddingIndex.currentChunks`의 조각 목록으로 BM25와 기존 의미 색인을 열고 BM25·임베딩·하이브리드의 후보와 근거를 출력합니다. 자기 청킹을 썼다면 **같은 선택 조각 목록을 두 색인에 전달**하도록 진입점을 맞춥니다. 출력의 `answer_generation=NOT_RUN`은 검색·발췌 비교이며 실제 생성 답변 비교가 아닙니다.

비교 입력은 “이중 결제 환불 접수 기한은 며칠인가요?”, `golden.json`의 `paraphrase`, “환불 접수 기한과 계정 잠금 해제 방법을 알려 주세요”입니다. 원문·필터·후보 상한·최종 문맥 수를 맞추고 실제 입력별 차이를 읽습니다. 하이브리드가 나아지지 않아도 그 결과와 구성 비용을 선택의 근거로 남깁니다.

```text
현재 RAG의 자료와 청킹을 유지해 실제 BM25와 하이브리드 검색을 연결해주세요.
제공 Lucene 구현과 기존 임베딩 저장소를 활용하고 검색 점수 대신 순위를 결합하는 이유를 설명해주세요.
필터·후보 수·문맥 선택을 맞출 구성을 추천하고 내 의견 뒤 구현해주세요.
제공 세 질문에서 후보·근거의 차이를 실제 실행으로 확인하고 선택한 검색을 기존 답변 경로에 연결해주세요.
검색 성공과 생성 답변의 정확성을 구별하고 새 프레임워크나 검색 서버 운영은 추가하지 마세요.
```

**남길 결과·완료 판단:** 실제 BM25·임베딩·하이브리드 비교와 선택 이유, 선택한 검색이 기존 근거·답변 경로로 이어지는 코드입니다. 가짜 벡터 테스트는 결합·필터를 확인하며 실제 임베딩 품질 확인을 대신하지 않습니다.
...
Day 3에서 선택한 검색으로 기존 골든 질문을 실행합니다. `SearchComparison.main`에 `--live --answer`를 지정하면 각 검색의 후보를 `Answering.answer`로 문맥 구성한 뒤 기존 `Quickstart.generateLive`와 `finish`의 생성·출처 검사로 이어집니다. 한 질문에 세 검색 방식의 답변을 생성하므로 필요한 질문만 실행합니다. 자기 앱에서도 선택한 검색을 기존 생성 경계에 연결하며 이미 있는 최종 출력 스키마·출처 검사를 재사용합니다.

실패가 있다면 위 표로 위치를 찾고 **한 가지 변경**을 선택합니다. 예를 들어 질문을 재작성한다면 원래 질문과 검색용 질문을 각각 보관하고 검색 결과만 바꾼 뒤 최종 답변에는 원래 질문을 사용합니다. “환불 접수 기한”을 “환불 확정 시점”으로 바꾸면 검색이 잘되어도 요구가 바뀝니다. 실패가 없으면 개선을 꾸며내지 말고 현재 방식이 충분한 근거와 다른 방식을 도입할 조건을 설명합니다.

```text
현재 실제 검색·답변에서 실패한 사례가 있으면 후보·문맥·생성 중 원인을 찾아주세요.
원인에 맞는 변경 하나를 추천하고 내 예상 뒤 적용해 같은 질문과 기존 골든을 확인해주세요.
질문 의도·접근 범위·출처가 유지됐는지, 개선되지 않은 부분과 비용도 함께 해석해주세요.
이미 충분한 결과라면 불필요한 개선 구현을 만들지 말고 그 판단 근거를 남겨주세요.
```

문서 개정 후에는 의미 색인을 갱신하고 현재 문서로 BM25도 다시 구성합니다. 모델이나 프롬프트까지 바꾸는 비교는 검색 변경과 분리해 어떤 변경의 효과인지 알 수 있게 합니다. 답변의 문구가 달라도 원문 조건을 지키는지 같은 기준으로 판단합니다.
...
자기 앱의 **근거에서 생성·결과 판단으로 가는 경계**를 제공 `Evaluation.evaluateEvidence`에 전달합니다. 선택한 형식·출처·보류 처리까지 실행하고, 사실의 지지는 실제 답변과 원문을 대조합니다. 예를 들어 아래 `app.answerFromSources`는 앱이 선택한 계약에 대응하는 어댑터이며 이름은 실제 구현에 맞춥니다.
...
권한·신뢰 필터는 기존 `PipelineTest.accessAndTrustFiltersPrecedeScoring`을 읽어 확인합니다. 이 검사는 `tenant-alpha`의 요청에서 다른 테넌트 자료와 불신 자료가 후보에서 제외되는지 봅니다. 검색에서 자료를 제외하는 필터의 역할과, 전달된 자료 속 행동 지시를 처리하는 생성 경계의 역할을 두 Red Team 결과와 연결해 설명합니다.
...
- 같은 문서·조각·접근 조건에서 실제 BM25·임베딩·하이브리드를 실행하고 선택한 검색을 답변 경로에 연결했습니다.
- 검색 변경 전후의 근거·실제 답변을 비교하고 관찰된 실패의 개선 위치 또는 현재 구성을 유지할 이유를 설명했습니다.
...
다섯 결과를 한 표에 모읍니다. `RequirementTest`의 5개 사례는 기능의 수용 기준이며 학습자의 이해 점수가 아닙니다.
...
하네스 v2는 **5주차에서 연결한 맥락·작업 규칙·도구·환경·검사·피드백과 자체 실행 엔진을 비교 결과에 맞춰 개선한 구성**입니다. 작업 계약·상태·검사·복구·종료의 기본 흐름은 v1에서 완성합니다. 이번에는 관찰된 문제가 어느 구성에서 생겼는지 판단해 필요한 작업 규칙·인계·정책·실행 제어를 바꿉니다. 기준 전달이 문제라면 맥락·지침을, 검사 누락이라면 수용 근거를 살피며 모든 개선을 엔진의 새 분기로 만들지는 않습니다. 기존 v1에 이미 필요한 통제가 있다면 그것을 유지할 근거와 불필요한 단계의 축소를 v2의 설계로 삼을 수 있습니다. 다섯 방식 모두 잘되었다면 가짜 결함을 만들지 말고 관찰한 분담·진행 비용에서 선택 기준을 정합니다.
...
**남길 결과·완료 판단:** M1~M5의 실제 비교와 v2의 선택·변경·확인 근거를 `tool-choice.md`에 연결합니다.

### 개발 방식의 품질과 AI 기능의 품질

이번 M1~M5 비교는 AI를 사용해 **코드를 개발하는 방식**의 비교입니다. 앱이 고객에게 내는 답변의 근거 정확성이나 도구 선택 품질과는 다른 결과입니다. 11주차에서는 이번에 선택한 지침·검사·개입 방식을 사용하면서 앱의 동일 입력·원문·도구 기록을 비교합니다. 역할 수나 사용량이 늘었다는 사실만으로 개발 결과나 답변 품질이 좋아졌다고 판단하지 않습니다.

포트폴리오에서 범위가 커지면 작업 간 필요한 정보, 반환 계약, 통합에서 생긴 오류를 보고 분업을 조정합니다. 모든 개발 방식을 큰 앱으로 다시 구현하거나 일정 토큰 사용량을 채우는 조건은 두지 않습니다. 실제로 달라진 품질·수정 비용을 근거로 선택한 방법을 재사용합니다.
...
새로운 업무 요구를 받았을 때, 앞서 배운 기술을 어떤 책임으로 나누고 연결해야 실제로 쓸 수 있을까요? 이번 주에는 **주문 조회와 정책 검색을 결합해 고객 문의에 답하는 단일 에이전트**를 AI와 설계·구현합니다. 자연어 요청에서 필요한 정보를 확인하고, 도구를 사용하고, 근거를 붙여 답하거나 추가 질문을 하는 흐름을 완성합니다.

7주차는 같은 업무를 여러 구성으로 표현하며 프레임워크의 역할을 비교했습니다. 8주차는 검색·답변과 근거의 품질을 확인했습니다. 여기서는 새로운 요구에서 어떤 도구와 상태가 필요한지 정하고 그 경계를 하나의 애플리케이션에 연결합니다. 10주차에서 정한 개발 지침·검증 방식을 작업에 활용하되 별도 비교 실험을 다시 만들지 않습니다.

## 제공 업무와 자료

가상의 쇼핑몰 상담 업무입니다. 입력은 자연어 문의와 대화 식별자이고, 결과는 답변·정책 출처·실제 도구 호출 기록입니다. 처음에는 한 주문을 처리하고 Day 5에서 여러 주문으로 확장합니다. 실제 고객 정보나 외부 프로젝트는 필요하지 않습니다.

| 자료·입력 | 확인할 내용 |
|---|---|
| `A-102` | 배송 준비, 출고 전 |
| `A-103` | 배송 중, 출고됨 |
| `A-104` | 배송 완료, 출고됨. 수령일과 사용 여부는 자료에 없음 |
| `A-999` | 제공 주문에서 조회되지 않음 |
| `POL-CANCEL` | 출고 전 취소 신청 조건 |
| `POL-RETURN` | 배송 완료 후 7일 이내·미사용 조건, 배송 중 안내 |
| `POL-DELIVERY` | 정확한 도착 시각을 약속할 근거가 없음 |

“A-102가 배송 전인데 취소할 수 있나요?”에서는 주문 조회로 출고 여부를 확인하고 정책 검색으로 신청 조건을 확인합니다. “A-102의 상태만 알려 주세요”에는 주문 조회만으로 필요한 사실을 얻습니다. “취소할 수 있나요?”에는 주문 번호가 없어 질문이 필요합니다. 취소·반품을 실행하는 업무는 없으므로 신청을 완료했다고 답하면 잘못된 결과입니다.

## 개념과 설계의 출발점

### 파이프라인

입력에서 결과까지 연결된 처리 단계입니다. `pipeline_example.py`의 `cancellation_pipeline`은 주문 조회가 성공하면 `shipped` 값으로 검색어를 고르고 정책 검색을 수행합니다. 순서와 분기를 코드가 정합니다. 입력이 항상 주문 번호가 있는 취소 문의라면 이 구성이 간단하고 예상하기 쉽습니다.
...
상태만 묻는 요청도 받으려면 고정 흐름에는 문의 종류를 나누는 분기가 필요합니다. 정보가 부족한 자연어 문의를 받으려면 질문과 재개도 표현해야 합니다. 업무 종류가 정해져 있고 처리 순서가 중요하면 고정 흐름이 적합하고, 다양한 요청에 따라 도구를 선택해야 하면 에이전트 구성이 도움이 됩니다. 짧은 비교 예제는 이 선택을 읽기 위한 것이며 두 앱을 각각 완성하는 과제가 아닙니다.

### 에이전트와 실행 제어

에이전트는 모델이 현재 입력과 도구 결과를 보고 다음 도구나 최종 응답을 선택하는 구성입니다. 참고 구현의 `create_agent`가 모델 호출 → 도구 호출 → 결과를 모델에 전달하는 반복을 실행합니다. 실제 업무 함수는 애플리케이션이 수행하고, 호출 제한도 애플리케이션이 적용합니다. 모델이 도구 호출을 제안했다는 사실과 도구 실행이 성공했다는 사실은 구별해야 합니다.

`agent.py`는 `make_tools`로 함수의 이름·설명·입출력을 모델에 공개하고 `CustomerAgent`에서 이를 등록합니다. SDK가 요구하는 도구 호출 형식과 `thread_id`는 연결 계약입니다. 주문과 정책을 별도 도구로 둘지, 어느 조건에 질문할지, 무엇을 기록할지는 업무 때문에 정한 설계입니다. 프레임워크가 이 판단까지 대신 정해 주지는 않습니다.

### 대화 상태

“취소할 수 있나요?” → “주문 번호를 알려 주세요” → “A-102예요”를 연결하려면 번호뿐 아니라 원래 문의도 남아 있어야 합니다. 참고 구현은 `InMemorySaver`와 대화 식별자로 메시지를 이어갑니다. 같은 식별자는 같은 대화이고 다른 식별자는 별도 대화입니다. 메모리 상태이므로 앱 재시작 후에는 이어지지 않습니다.

대화를 이어간다고 조회 사실을 계속 최신으로 간주할 수는 없습니다. 참고 지침은 현재 요청의 주문과 정책을 다시 조회하게 합니다. “이번에는 A-103은요?”에는 앞선 A-102 사실을 복사하지 않고 새 대상을 조회해야 합니다. 장기간 보존이 필요해지면 저장소와 만료 정책을 선택하겠지만, 이번 사례는 실행 중의 대화 이어받기를 확인합니다.

### RAG와 도구의 연결

검색 도구는 정책의 원문·출처를 반환하고 모델이 주문 사실과 함께 해석합니다. `PolicySearch`는 문서를 임베딩하여 `InMemoryVectorStore`에 넣고 질의와 가까운 문서를 찾습니다. 짧은 제공 정책 한 항목을 한 문서로 사용합니다. 긴 매뉴얼로 바뀌면 분할 단위·출처 범위·색인 갱신 방법을 다시 선택해야 합니다.

예를 들어 A-104가 배송 완료라는 사실만으로는 반품 가능 여부를 확정할 수 없습니다. 검색된 `POL-RETURN`에는 **7일 이내와 미사용**이라는 조건이 있으므로 수령일과 사용 여부를 확인해야 합니다. 검색 성공은 필요한 근거를 얻을 기회이며, 그 조건이 실제 주문에서 충족된다는 증명은 아닙니다.

## 작업 위치와 준비

기본 작업 폴더는 `week11-ai-agent-integration/agent_lab`입니다. 주차 노트는 `week11-ai-agent-integration/notes.md` 또는 이미 사용하는 노트 한 곳에 누적합니다. 코드·설정·실제 실행 결과도 산출물입니다.

| 파일 | 실제 역할 |
|---|---|
| `data/orders.json`, `data/policies.json` | 제공 주문 사실과 정책 원문 |
| `business.py` | 대화와 분리된 주문 조회·벡터 검색 |
| `agent.py` | 도구 공개, 프레임워크 연결, 대화 상태·종료·응답 처리 |
| `model_boundary.py`, `app.py` | 실제 연결 경계와 IDE 실행 진입점 |
| `pipeline_example.py` | 고정된 처리 순서를 읽는 비교 예제 |
| `offline.py`, `tests/test_agent.py` | 고정 응답 재생과 연결·실패 처리 검사 |

IDE에서 Python 3.11 이상 가상환경을 선택하고 `requirements.txt`의 의존성을 설치합니다. 의존성을 한 번에 설치하는 터미널 명령은 Windows PowerShell·macOS·Linux·WSL 공통으로 다음과 같습니다. 이후 편집·실행·테스트는 IDE 화면에서 진행합니다.

```text
python -m pip install -r requirements.txt
```

`app.py`의 기본 실행은 고정 응답 재생입니다. 실제 연결은 IDE 실행 인수에 `--live`를 넣고 비공유 실행 설정에 `AI_AX_LIVE=1`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_EMBEDDING_MODEL`을 지정한 뒤 학습자가 실행합니다. 계정에서 사용할 수 있는 대화·임베딩 모델 이름을 지정합니다. 시작 시 문서 임베딩, 검색 시 질의 임베딩과 모델 호출에 비용이 발생합니다. 실제 키나 실행 설정은 AI에게 보여 주지 않고 파일로 자동 로딩하지 않습니다. AI의 검증은 설정·모델·임베딩을 가짜 객체로 바꿔 수행합니다.

참고 코드는 시작부터 열어 재사용할 수 있습니다. 제공 업무 함수와 SDK를 활용해 자신의 구조로 구현하거나 참고 구현을 수정해 이어가세요. 다른 진입점·도구 계약을 선택해도 같은 업무 입력과 결과로 확인합니다. `CustomerAgent`라는 클래스나 특정 빈 함수를 채우는 것이 학습 목표는 아닙니다.

## 실행 자료의 추가 역할

`flow_patterns.py`는 고정 분기·독립 조회 통합·한 번의 검토 후 수정이라는 처리 구조를 보여 줍니다. `data/quality_cases.json`은 답변과 도구 선택을 비교할 소수 입력, `compare_answers.py`는 같은 사례의 두 프롬프트 결과를 나란히 읽는 참고 진입점입니다. 완료할 앱은 주문 상담 앱 하나이며 구조 비교 예제를 모두 별도 앱으로 만들지 않습니다.

## Day 1 — 요구에서 책임과 흐름 도출하기

**할 일:** 제공 요청 세 가지인 취소 문의, 상태 조회, 번호 없는 문의를 읽고 필요한 기능을 구분합니다. 먼저 고정 파이프라인 예와 `agent.py`의 도구 등록·프레임워크 연결을 비교합니다. AI에게 실제 코드에 근거한 추천을 받고 자신의 선택을 짧게 말합니다.

주문 조회는 특정 번호로 정해진 사실을 찾고, 정책 검색은 질문과 관련된 문서를 찾습니다. 둘을 분리하면 상태 문의에 검색 비용을 쓰지 않아도 되고, 검색기를 바꿔도 주문 조회 계약을 유지할 수 있습니다. 반대로 문의가 항상 같은 종류이고 결과 형식이 엄격하면 하나의 고정 업무 함수로 묶는 편이 단순할 수 있습니다. 이번에는 다양한 자연어 문의를 받으므로 두 도구를 공개하는 에이전트를 기본 추천으로 삼습니다.

```text
이번 주 제공 주문·정책 상담 요구와 agent_lab의 코드·자료를 읽어주세요.
취소 문의, 상태 조회, 주문 번호 없는 문의를 각각 어떤 경로로 처리할지 설명해주세요.
입출력·도구의 책임·대화 상태·허용 동작·종료 조건을 실제 요구에서 도출하고,
참고 구현에서 선택한 구조와 SDK가 요구하는 계약을 구분해주세요.
추천하는 구성과 이유, 같은 입력의 예상 결과를 먼저 보여주세요.
내 수용·수정 의견을 반영해 이번 주 사용할 구현 위치와 첫 기능을 정하겠습니다.
참고 클래스나 빈 연결부를 채우는 방식만 정답으로 삼지 마세요.
```

**남길 결과·완료 판단:** 선택한 구성과 “A-102 취소 문의”의 예상 처리 경로를 주차 노트에 남깁니다. 설계 문서를 먼저 완성할 필요는 없으며, AI의 설명과 자신의 의견을 구분합니다.

### 다른 요구에서 처리 구조 고르기

| 제공 요구 | 가능한 구조 | 전달할 중간 결과와 한계 |
|---|---|---|
| 상태 조회는 주문만, 취소 문의는 정책까지 확인 | 고정 라우팅 또는 모델의 도구 선택 | 문의 종류가 불분명하면 추가 질문. 분류가 틀리면 뒤 단계도 틀릴 수 있음 |
| 여러 주문을 각각 확인하고 합쳐 안내 | 독립 조회의 병렬 처리와 주문별 통합 | 각 사실을 주문 ID에 연결. 한 주문의 결과가 다른 조회의 입력이면 순서를 보장해야 함 |
| 답변에 정책 근거가 없으면 한 번만 보완 | 검사 → 피드백 → 제한된 수정 → 재확인 | 출처 ID 존재 검사와 내용의 타당성 판단을 구분. 계속 실패하면 보류 |
| 검토한 내용을 실제 신청으로 실행 | 사람 확인 대기와 실행 결과 저장 | 승인 대상과 실행할 내용을 연결하고 재요청의 중복 효과를 정함 |
| 서로 다른 업무 전문 판단이 필요 | 전문 처리기로 라우팅하거나 여러 에이전트의 결과 통합 | 맡길 입력·반환 계약·불일치 해결·전체 호출 예산 필요 |

IDE에서 `flow_patterns.py`를 실행하고 `route`, `collect_orders`, `revise_once`의 입력에서 결과까지 읽습니다. `A-999`가 조회되지 않아도 다른 주문이 남는 이유는 독립된 조회와 오류 처리 위치 때문입니다. 출처가 없는 초안을 그대로 돌려주는 수정 함수는 한 번 후 `needs_review`로 끝납니다. 횟수 제한이 내용의 개선까지 보장하지 않음을 보여 주는 고정 예입니다.

개발 세션에 개발자·QA 역할을 주는 것과 앱 내부의 여러 모델을 구성하는 것은 구분합니다. 이 예제는 **여러 모델 에이전트나 LLM 평가자를 구현한 것이 아닙니다.** 전문 판단 분리가 필요한 요구가 생기면 각 처리기의 결과 계약을 먼저 고르고 실제 모델 호출로 확인합니다. 이번 상담 앱에 모든 구조를 추가하지 않습니다.

```text
주문 상담 요구에 대해 고정 분기, 모델 도구 선택, 독립 조회 통합 중 필요한 구성을 추천해주세요.
flow_patterns.py의 실제 입력·중간 결과를 사용해 차이와 적용 조건을 설명해주세요.
전문 에이전트나 검토 후 수정이 필요한 조건도 설명하되 이번 요구에서 선택한 구조만 구현해주세요.
내 수용·수정 의견 뒤 도구·상태·종료 정책과 기존 참고 코드의 연결 위치를 정해주세요.
```

## Day 2 — 실제 모델과 주문 조회 연결하기

**할 일:** 먼저 “A-102의 상태만 알려 주세요”를 처리합니다. 선택한 구조에서 모델과 주문 조회 도구를 연결하고 학습자가 실제 모델로 실행합니다. 참고 구현을 쓰면 `CustomerAgent(model, lookup)`처럼 검색 도구 없이 첫 경로를 살필 수 있습니다.

`OrderLookup`은 입력을 정규화한 뒤 `found`와 `not_found`를 구분해 반환합니다. `make_tools`의 `lookup_order`는 이를 모델에 공개합니다. `create_agent`에 등록한 함수는 모델이 이름과 인수를 선택했을 때 호출됩니다. 출력의 `tool_calls`에서 `name`, `args`, `result`를 읽으면 모델이 어떤 요청을 했고 실제로 무엇을 받았는지 확인할 수 있습니다. 이 기록은 모델의 내부 사고 과정이 아닙니다.

```text
Day 1에서 정한 구성으로 자연어 상태 문의→주문 조회→응답을 연결해주세요.
제공 업무 함수·실제 LangChain과 SDK·참고 코드는 재사용해도 됩니다.
새 책임이나 계약을 선택해야 한다면 추천·이유·예상 결과를 먼저 설명하고 내 의견을 반영하세요.
이미 정한 선택 안의 구현과 오류 수정은 계속 진행해주세요.
AI 검증에는 가짜 설정과 모델을 사용하고, 실제 연결 설정은 읽거나 실행하지 마세요.
내가 IDE에서 실제 모델로 실행할 입력과 확인할 도구 기록을 안내해주세요.
실행 결과가 오면 입력부터 결과까지 핵심 코드에 연결해 해석해주세요.
```

예상되는 결과는 배송 준비라는 제공 사실이며, 특정 도착일을 약속할 근거는 없습니다. 도구 호출이 없이 모델이 우연히 같은 답을 했다면 실제 조회 연결을 확인한 것이 아닙니다. 반대로 도구는 성공했는데 답변이 잘못되면 조회 코드와 답변 생성 정책을 분리해서 원인을 살펴봅니다.

**남길 결과·완료 판단:** 실제 모델의 조회 호출과 답변을 확인하고 그 경로를 핵심 코드에서 찾습니다. 고정 응답 재생만 했다면 구조 확인까지 기록하고 실제 연결은 미확인으로 남깁니다.

## Day 3 — 주문 사실과 정책 근거 통합하기

**할 일:** 정책 검색 도구를 연결하고 “A-102가 배송 전인데 취소할 수 있나요?”를 실행합니다. 상태만 묻는 요청과 비교해 필요한 도구가 달라지는지 봅니다.

`business.py`의 검색 결과는 `source_id`, `title`, `text`, `score`를 포함합니다. 점수는 현재 임베딩의 유사도이며 정책의 정확성이나 신청 가능 확률이 아닙니다. 참고값 `min_score=0.3` 아래는 제외하고 최대 두 문서를 전달합니다. 이 값은 제공 사례를 시작하기 위한 선택이며 실제 모델에서 관련 없는 문서가 섞이거나 필요한 문서가 빠지는지 확인해야 합니다.

`POL-CANCEL`은 출고 전이라는 조건을, 주문 자료는 A-102가 출고 전이라는 사실을 줍니다. 두 근거를 합쳐 취소를 **신청할 수 있다**고 답할 수 있지만, 신청을 완료했다고 답할 수는 없습니다. `agent.py`는 답변의 `[POL-CANCEL]`이 이번 검색 결과에 존재하는지 검사합니다. 없는 출처 때문에 답변을 보류하면 저장된 최종 메시지도 보류 문구로 교체합니다. 반환값만 바꾸면 다음 대화에서 모델이 보류된 원문을 사실처럼 다시 사용할 수 있기 때문입니다. 존재하는 출처를 붙여도 본문과 다른 주장을 할 수 있으므로 의미 일치는 실제 답변과 원문을 대조합니다.

```text
현재 앱에 제공 정책 검색을 연결해주세요. 주문 조회와 검색 결과가 답변에 전달되는 경로를 설명해주세요.
도구 입출력·출처 표시·검색 실패 처리에 새 선택이 필요하면 추천과 이유를 먼저 보여주세요.
내 의견을 반영한 뒤 기존 결과물에 구현하고 가짜 모델·임베딩으로 연결을 확인해주세요.
실제 모델에서는 상태 조회와 취소 문의를 각각 실행하겠습니다.
내가 가져온 도구 기록·답변을 바탕으로 검색 필요성, 주문 조건과 정책 근거의 결합을 해석해주세요.
출처 ID가 있다는 이유만으로 답변이 정확하다고 판단하지 마세요.
```

**남길 결과·완료 판단:** 주문 사실과 검색된 정책이 답변에 연결되고 실제 출처를 확인합니다. 필요한 검색을 생략하거나 상태 조회에 불필요한 검색을 하는 경우 지침·도구 설명과 실제 선택의 관계를 살펴 수정합니다.

## Day 4 — 대화와 실패 상황에서 동작 확인하기

**할 일:** 아래의 작은 제공 사례를 실행하고 설계한 질문·재개·실패·종료 정책을 대조합니다. 도구 장애와 호출 제한은 가짜 경계로 재현하고, 자연어 후속 질문과 근거 사용은 실제 모델에서 확인합니다.

| 입력·조건 | 기대하는 처리 |
|---|---|
| 같은 대화에서 “취소할 수 있나요?” → “A-102예요” | 번호를 묻고 원래 취소 문의를 이어 처리 |
| 같은 대화에서 “이번에는 A-103은요?” | 새 주문을 조회하고 출고 전 취소와 구별 |
| 다른 대화에서 “그 주문은요?” | 다른 대화의 주문을 가져오지 않고 확인 질문 |
| “A-999를 취소할 수 있나요?” | 조회되지 않았다고 설명하고 사실을 만들지 않음 |
| “A-104를 반품할 수 있나요?” | 정책 조건에 필요한 수령일·사용 여부 확인 |
| 제공 정책에 없는 보증 조건 문의 | 관련 문서가 검색돼도 없는 조건을 확정하지 않음 |
| 검색기가 빈 목록을 반환 / 호출 오류 발생 | 근거 부족과 호출 실패를 구분 |
| 가짜 모델이 계속 도구 호출 제안 | 여섯 번의 모델 호출 뒤 한도 종료 |

참고 구현의 `RequestBudget`은 LangChain의 모델 호출 제한을 사용하고 한도 도달 응답을 한국어로 바꿉니다. 모델에 “적게 호출하라”고 부탁하는 것과 달리 일곱 번째 호출 전에 실행을 종료합니다. 이 한도는 요청마다 새로 적용됩니다. 한 모델 응답에 여러 도구 호출이 포함될 수 있으므로 모델 호출 수와 도구 호출 수는 같은 수치가 아닙니다.

오류 시 `unavailable`은 실행 실패이고 `not_found`는 정상 조회 후 결과 없음입니다. 임베딩 호출 장애를 “해당 정책이 없다”로 바꾸면 업무 판단이 잘못됩니다. 참고 앱은 실패한 실행의 부분 메시지를 이전 대화로 복원해 다시 문의할 수 있게 합니다. 현재 도구가 조회뿐이어서 가능한 단순 정책이며, 실제 저장·결제 같은 기능을 추가한다면 실행 여부·중복 처리와 재시도 기준을 별도로 설계해야 합니다.

```text
현재 앱의 Day 4 제공 사례를 기준으로 상태·실패·종료 처리를 점검해주세요.
설계가 더 필요한 지점만 코드·입력에 근거한 추천과 예상으로 설명하고 내 의견을 반영하세요.
가짜 모델·임베딩·설정으로 오류와 한도를 재현하고, 학습자가 실제 연결에서 확인할 사례를 구별해주세요.
실제 결과가 오면 같은 대화의 재개, 다른 대화의 분리, 주문 변경, 정보 부족 처리를 해석해주세요.
노트는 기존 한 곳에 누적하고 실제 모델에서 확인하지 않은 선택을 확인한 것으로 쓰지 마세요.
```

**남길 결과·완료 판단:** 대표 사례의 결과와 관련 정책·코드를 연결해 설명합니다. `responded`라는 출력 상태는 응답을 얻었다는 뜻이며 문의 해결이나 학습 완료 점수가 아닙니다.

### 실패 처리의 확장 조건

지금 앱은 조회 중심이며 실행 중 메모리로 대화를 구분합니다. 재시작 후 이어가기 요구가 생기면 지속 체크포인트에 같은 대화 ID를 연결하고, 실제 프로세스 재시작 뒤 대기 지점·원래 문의가 복원되는지 확인합니다. 저장된 과거 주문 사실의 재조회·보존 기한도 함께 정해야 합니다.

실제 쓰기 기능을 추가한다면 9주차처럼 사람이 확인한 대상·내용과 실행을 연결하고, 응답을 못 받았을 때 실행 여부를 확인할 방법을 둡니다. 반복 호출 상한만으로 중복 신청을 막지는 못합니다. 이 주차의 조회 앱에 지속 저장소나 신청 기능을 추가하는 공통 과제로 바꾸지는 않습니다.

## Day 5 — 여러 주문과 부분 성공 요구 적용하기

**할 일:** “A-102와 A-103, A-999는 각각 취소할 수 있나요?”라는 후속 요구를 기존 구현에 적용합니다. 단일 결과를 덮어쓰는 부분이 있는지, 조회 실패 시 전체 처리를 중단하는지, 근거를 어느 주문에 붙이는지 살펴봅니다.

이 요구는 입력값만 바꾸는 활동이 아닙니다. 한 번의 조회 결과를 담던 구조라면 주문별 결과를 유지하도록 바꿔야 하고, 전체 중단 정책이라면 부분 성공을 표현해야 합니다. 참고 구현은 도구 결과를 호출별로 보존하므로 이미 여러 호출을 담을 수 있습니다. 이를 재사용한다면 왜 구조를 크게 바꾸지 않아도 되는지 실제 메시지와 기록에서 확인하고, 자신의 이전 구현에 필요한 변경을 적용합니다.

예상되는 해석은 A-102의 출고 전 취소 신청, A-103의 출고 후 안내, A-999의 조회 불가입니다. 조회되지 않은 주문이 앞에 오거나 뒤에 와도 확인된 다른 결과를 버리면 안 됩니다. 도구 호출 순서가 같아야 한다는 요구는 없으며 주문별 사실과 정책 적용이 맞아야 합니다.

```text
현재 구현에 여러 주문을 한 번에 문의하고 일부만 조회돼도 확인된 결과는 답하는 요구를 적용해주세요.
A-102와 A-103, A-999의 취소 문의를 기준으로 현재 계약·상태·실패 정책에서 바뀔 부분을 설명해주세요.
이미 이 요구를 처리하는 구조라면 실제 코드와 메시지로 그 이유를 보여주세요.
필요한 변경의 추천·이유·예상을 내 의견과 연결한 뒤 같은 결과물에 구현해주세요.
가짜 경계로 부분 성공을 확인하고 내가 실제 모델로 확인할 입력을 안내해주세요.
실제 결과와 선택 이유를 기존 주차 노트에 정리하고, AI가 작성한 설명을 내 판단으로 바꾸지 마세요.
```

**남길 결과·완료 판단:** 후속 요구가 반영된 앱과 실제 결과, 유지하거나 변경한 책임·상태·정책의 이유를 남깁니다. 블로그로 정리한다면 이 근거를 사용하며 별도 보고서나 공개를 의무로 삼지 않습니다.

## 다른 요구에서 기술을 고르는 기준

MCP는 주문 조회나 정책 검색을 여러 Host에서 사용할 공통 도구로 공개할 때 선택할 수 있습니다. 현재 앱 내부에서만 쓰는 함수를 모두 서버로 옮길 이유는 없습니다. Skill은 AI 개발 도구에서 반복할 검토·구현 지침을 재사용할 때 도움이 됩니다. 앱의 고객 대화 상태와 개발 도구의 세션은 서로 다른 실행 맥락입니다.

업무 단계와 승인 순서가 고정되면 명시적인 파이프라인이나 LangGraph의 상태·분기가 더 적합할 수 있습니다. 현업 사용자가 화면에서 흐름을 조정해야 하면 Dify와 코드 도구의 경계를 검토합니다. 어떤 요구 때문에 선택이 바뀌는지를 설명하고, 이번 앱에 모든 기술을 추가하는 과제로 바꾸지 않습니다.

12주차에는 원하는 포트폴리오의 목적과 범위를 정합니다. 이번 주의 업무·Python·에이전트 형태를 유지할 필요는 없습니다. 필요한 기능과 데이터, 결과를 확인할 방법을 도출한 경험을 사용합니다.

### 같은 입력으로 답변과 도구 선택 비교하기

`data/quality_cases.json`의 상태 조회·취소 문의·추가 질문·여러 주문 사례를 사용합니다. 바꿀 것은 한 번에 프롬프트, 모델 또는 도구 설명 중 하나입니다. 기본 안내와 “주문별 확인 사실·정책 조건·남은 확인을 묶어 쓰기”를 추가한 안내를 비교하는 참고 실행을 제공합니다. 어느 쪽이 더 좋다고 가정하지 않습니다.

IDE에서 `compare_answers.py`를 실행하면 입력과 판단 기준만 표시합니다. `--live`는 실제 모델·임베딩을 사용해 두 프롬프트에서 같은 네 사례를 실행합니다. 다른 한 가지 변경을 이미 선택했다면 그 변경과 같은 입력을 자기 진입점으로 비교하면 됩니다. 각 구성은 별도 대화 상태로 시작하고 보충 입력만 같은 대화에서 이어갑니다.

반환된 `missing_tools`는 필요한 도구 종류가 호출되지 않았다는 참고 표시이며 인자·호출 순서·의미 정확성의 점수가 아닙니다. 주문별 사실, 출처 원문, 신청 완료로 오해할 문구, 추가 질문의 필요성을 직접 읽습니다. 같은 결과면 더 단순한 구성을 유지할 근거가 됩니다. `completed_model_calls`는 반환받은 모델 응답 수이고 앱이 만든 상한 종료 안내는 제외합니다. `token_usage=null`은 미제공이며 0원이 아닙니다. 시간은 검색·도구까지 포함한 해당 실행의 관찰이고 소수 실행으로 보편적인 속도 우위를 주장하지 않습니다.

```text
현재 앱에서 프롬프트·모델·도구 설명 중 바꿀 하나와 이유를 추천해주세요.
내 예상 뒤 제공 품질 사례를 같은 조건에서 실행할 방법을 준비해주세요.
실제 답변·출처·도구 인자와 완료된 모델 호출 수를 비교하고, 개선되지 않은 결과도 남겨주세요.
도구 이름 확인이나 자동 검사만으로 답변 의미가 맞다고 채점하지 마세요.
선택한 변경을 유지하거나 되돌릴 이유와 기존 실패 사례의 결과를 주차 노트에 정리해주세요.
```

**남길 결과·완료 판단:** 실제 입력별 변경 전후 결과와 학습자의 해석, 최종 선택입니다. 원래 Day 5의 여러 주문 입력을 재사용하고 별도 채점표·대량 평가 시스템을 만들지 않습니다.

## 완료 기준

- 처리 분기·독립 작업 통합·검토 후 수정의 적용 조건을 실제 비교 예에서 설명하고 선택한 구조를 앱에 연결했습니다.
- 같은 소수 입력에서 한 가지 변경 전후의 실제 답변·출처·도구 선택을 비교하고 최종 선택을 해석했습니다.

- 제공 요구에서 도구·입출력·대화 상태·실패·종료를 선택한 이유를 핵심 코드와 연결했습니다.
- 실제 LLM과 주문 조회·정책 검색을 연결하고 답변과 정책 근거를 확인했습니다.
- 추가 질문과 재개·실패·호출 제한·대화 분리를 해당 확인 방법으로 점검했습니다.
- 여러 주문의 부분 성공 요구를 기존 구현에 적용하고 실제 결과와 설계 판단을 설명했습니다.

가짜 모델·임베딩 검사는 연결과 실행 정책을 확인합니다. 실제 모델의 도구 선택·자연어 답변·검색 품질은 학습자가 실제 연결에서 확인해야 합니다. 실행하지 못한 부분은 미확인으로 남깁니다.

참고: [LangChain Agents](https://docs.langchain.com/oss/python/langchain/agents), [모델 호출 제한](https://docs.langchain.com/oss/python/langchain/middleware/built-in#model-call-limit), [메모리 벡터 저장소](https://docs.langchain.com/oss/python/integrations/vectorstores/in_memory). API 계약을 확인할 때 사용하고, 본문의 설계 판단과 제공 사례를 함께 읽습니다.

<!-- MODULE:11 END -->

<!-- MODULE:12 START -->
# 12주차 — 원하는 포트폴리오 구체화와 첫 구현
...
검증도 그 목적에 맞춥니다. 실행 결과·스크린샷·데모·사용 예·기존 테스트 중 동작을 가장 잘 보여 주는 근거를 사용합니다. AI가 초안을 썼다는 사실은 학습자가 구조를 이해했다는 근거가 아니므로 주요 선택과 실제 결과를 연결해 말할 수 있어야 합니다.

## 작업 공간과 준비

`week12-portfolio-build`의 README에서 진행합니다. 결과물 코드는 학습자가 고른 위치에서 만들거나 기존 위치를 이어 사용할 수 있습니다. 저장소·폴더 이름과 내부 구조를 고정하지 않습니다. 현재 주차 노트에는 선택한 작업 위치를 상대경로나 자리표시자로 연결하고, 다른 저장소의 개인 자료를 과정 원본에 복사하지 않습니다.
...
### AI 프로젝트를 선택했을 때 구현 범위 정하기

주제·언어·결과물 형태는 원하는 것으로 정합니다. 제품에 LLM을 쓰는 프로젝트라면 “LLM API를 넣었다”에서 끝나지 않도록 **사용자 입력 → 필요한 자료·도구 → 모델 → 검사·다음 처리 → 사용 가능한 결과** 한 흐름을 완료 기준으로 정합니다. AI를 사용해 개발하는 프로젝트라면 개발 과정의 요구·선택·검증 근거를 그 목적에 맞게 정합니다.

| 선택한 프로젝트의 사용 장면 | 처음 연결할 범위 | 실제로 확인할 입력 |
|---|---|---|
| 문서 질문 서비스 | 자료 갱신 → 검색·근거 → 모델 답변 → 화면 또는 API | 표현이 다른 질문, 없는 근거, 개정된 정책 |
| 업무 분류·추출 자동화 | 입력 수집 → 구조화 출력 → 검사 → 저장 또는 후속 처리 | 누락·상충, 중간 호출 실패, 같은 항목 재전달 |
| 대화형 업무 도우미 | 대화 상태 → 필요한 도구 → 추가 질문·응답 | 번호 보충, 대상 정정, 다른 대화, 호출 상한 |
| 반복 업무 MCP 도구 | 기존 업무 기능 → MCP 계약 → 선택한 Host·앱 | 실제 도구 선택, 업무 오류, 연결 실패 |

이 표에서 자신의 목적에 맞는 경로만 고릅니다. 이미 아는 백엔드·배포 기술은 활용하고 AI 입출력·상태·실패가 그 구성과 만나는 지점을 설계합니다. 여러 경로를 모두 넣는 과제로 바꾸지 않습니다.
...
먼저 내부 함수의 결과 계약을 정하고 작은 제공·가상 입력으로 확인한 뒤 원하는 API·화면·작업 실행기에 연결합니다. 예를 들어 검색 답변 함수는 답변과 출처, 보완이 필요한 상태를 반환하고 HTTP 응답은 그 의미를 유지해 전달합니다. 원문 부족을 성공 빈 문자열로 바꾸면 화면은 무엇을 물어야 할지 알 수 없습니다.

자동 실행을 선택했다면 수집 항목의 ID·입력 버전·처리 결과를 연결합니다. 이미 처리한 항목을 건너뛸지 다시 모델에 보낼지는 정책입니다. 프롬프트·모델·원문이 바뀌었다면 이전 캐시를 그대로 사용할 조건도 달라집니다. 먼저 수동 한 번 실행으로 입력과 결과를 확인한 뒤 필요한 실행 계기나 주기를 붙입니다.

```text
선택한 프로젝트의 핵심 AI 기능을 실제 사용 경로에 연결해주세요.
현재 입력·결과·기술 스택을 보고 API, 화면 또는 작업 실행기와의 계약을 추천해주세요.
내 의견 뒤 한 흐름부터 구현하고 정상 입력과 가장 중요한 실패 입력을 직접 사용할 방법을 안내해주세요.
자료·프롬프트·모델 버전이 바뀔 때 상태와 캐시를 유지할 조건도 해당 기능에 필요한 만큼 설명해주세요.
기존 백엔드·배포 기술을 다시 배우는 과제나 모든 기술을 넣는 구조로 바꾸지 마세요.
```

**남길 결과·완료 판단:** 선택한 사용 경로로 실제 모델 결과를 받고 실패 상태를 구별한 구현·입력·결과입니다. API가 200을 반환했다는 사실만으로 답변이나 후속 업무의 성공을 판단하지 않습니다.
...
13주차에는 이 결과물에 필요한 보완을 고르고 우선순위를 함께 정하겠습니다.
사용성·실행 안내·데모·배포·공개 자료 중 목적에 필요한 것만 제안하세요.
이미 충족한 기준 때문에 새 기능이나 중복 문서를 만들지 마세요.
```

**남길 결과·완료 판단:** 첫 사용 가능한 흐름과 13주차에 수행할 선택된 작업을 남깁니다. 공개하지 않는 결과물도 실행·설명 목적에 맞으면 선택할 수 있습니다.
...
<!-- MODULE:13 START -->
# 13주차 — 선택한 포트폴리오 완성과 전달

## 중심 질문과 핵심 결과

선택한 결과물이 목적에 맞게 사용되고 설명되려면 무엇을 보완해야 할까요? 12주차에 정한 계획을 따라 **필요한 완성도와 전달 자료를 갖춘 결과물**을 만듭니다. 사용성·실행 안내·데모·배포·공개 자료 중 해당 포트폴리오에 필요한 작업을 고릅니다.
...
12주차에 선택한 입력·결과와 사용 장면을 다시 봅니다. 결과가 언제 유효한지, 잘못된 입력이나 자료 부족에서 어떻게 행동해야 하는지 목적에 필요한 범위를 정합니다. 외부 API를 사용하는 결과물이라면 호출 실패와 빈 업무 결과를 구별할 필요가 있고, 정적 결과물이라면 내용과 링크의 정확성이 더 중요할 수 있습니다.
...
12주차의 결과물과 작업 위치를 이어 사용합니다. `week13-portfolio-evidence`는 이번 안내를 찾는 위치이며 코드를 반드시 이 폴더로 옮기라는 뜻이 아닙니다. 기존 코드·지침·노트·진도는 유지합니다.
...
**할 일:** 12주차 말에 정한 남은 작업을 현재 동작과 대조합니다. 목적 달성을 막는 문제, 전달을 어렵게 하는 부분, 있으면 좋은 추가 기능을 구별합니다. 학습자가 선택할 수 있도록 AI가 영향과 필요한 노력을 구체적으로 설명합니다.

```text
12주차에서 선택한 목적·범위와 현재 결과물을 기준으로 마무리 작업을 정리해주세요.
...
| 실제 요구 | 구현·확인할 경계 |
|---|---|
| 답변을 기다리는 동안 진행 상황을 보여 줌 | 접수·검색 중·생성 중·완료·실패를 구분. 스트리밍의 마지막 완료 이벤트와 최종 출처까지 확인 |
| 사용자가 중간에 취소함 | 화면 취소와 실제 모델·도구 중단의 차이, 이미 실행된 쓰기의 상태를 안내 |
| 다른 대화·사용자가 같은 서버를 사용함 | 서버가 정한 사용자·대화 식별과 접근 범위를 도구·검색·상태에 전달. 모델이 보낸 ID만으로 권한을 결정하지 않음 |
| 앱을 재시작해도 작업을 이음 | 지속 상태에서 원래 입력·대기 지점·실행 결과를 복원하고 오래된 조회는 필요에 따라 갱신 |
| 같은 작업이 다시 실행됨 | 입력·작업 ID로 완료·미완료를 구분하고 중복 쓰기·불필요한 모델 호출을 통제 |
| 모델·프롬프트를 바꿈 | 기존 대표 입력과 실패 입력으로 품질·출처·도구 호출·관찰 가능한 사용량을 같은 기준으로 비교 |

표의 모든 기능이 필수는 아닙니다. 현재 프로젝트의 목적과 Day 1에서 정한 사용 조건에 해당하는 항목을 구현합니다. 스트리밍을 선택하면 서버가 델타를 전송하는 코드, 화면이 누적하는 코드, 실패·취소·최종 결과를 처리하는 위치까지 연결합니다. 완성 문자열을 잘라 보여 주는 것만으로 실제 모델 스트리밍을 구현했다고 하지 않습니다.

```text
현재 포트폴리오의 사용 조건에서 필요한 AI 연결 보완을 골라주세요.
진행 표시·취소·대화 분리·재시작·재실행 중 실제 요구에 해당하는 것을 추천하고 이유를 설명해주세요.
내 의견 뒤 선택한 항목만 기존 구조에 구현하고, 관련 정상·실패 입력으로 확인해주세요.
완료되지 않은 쓰기나 끊긴 모델 응답을 성공으로 보여 주지 않는지 실제 결과로 확인해주세요.
```

**남길 결과·완료 판단:** 선택한 조건에서 사용 가능한 결과와 실패 안내, 핵심 코드·설계 이유입니다. 공개 배포는 선택한 전달 방식에 필요할 때 수행합니다.
...
선택한 프로젝트에서 요구 하나, 중요한 구성 선택, 실제 실패와 수정 결과를 연결합니다. 10주차에서 고른 개발 방식이 큰 작업에서도 유효했는지, 11주차에서 익힌 입력·도구·근거·상태의 경계를 어디에 사용했는지 해당되는 부분을 설명합니다. 구현된 파일이나 AI가 써 준 해설의 존재를 자신의 설계 판단으로 바꾸지 않습니다. 블로그를 쓴다면 기존 노트와 실제 결과에서 이 근거를 가져오며 새 보고서를 중복 작성하지 않습니다.

## 완료 기준

- 12~13주차에서 선택한 목적과 범위에 필요한 결과물을 완성했습니다.
