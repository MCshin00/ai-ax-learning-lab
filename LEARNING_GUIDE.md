...
### 기본 노드와 Plugin의 경계 — 흐름의 분기와 업무 함수를 구분합니다

빈값 검사나 `status == ready` 같은 흐름 분기는 IF/ELSE 노드로 표현할 수 있습니다. 이 예제는 업무 필드·근거·상충을 검사하는 함수를 Tool Plugin으로 공개합니다. 같은 기능을 Code 노드에 둘 때와의 차이는 Day 3에서 실제 연결 코드와 함께 살펴봅니다.
...
Dify Cloud 또는 사용할 수 있는 Dify 환경과 연결할 모델을 준비하고, 선택한 계정·모델의 사용 비용을 확인합니다. Python 3.12와 Plugin 개발 연결은 Day 3에서 준비합니다.
...
LLM 노드의 Structured Outputs에서 `extraction-schema.json`의 JSON Schema를 지정합니다. 본문 프롬프트와 `회의 입력.memo` 연결도 유지합니다. 선택한 모델이 구조화 출력을 지원하는지 화면에서 확인하고 실제 노드 출력의 `structured_output` 객체를 봅니다. JSON을 부탁하는 프롬프트만 실행한 것은 이 기능의 확인과 구분합니다. UI·모델에 따라 지원 방식은 다를 수 있습니다. [Dify LLM 노드](https://docs.dify.ai/en/use-dify/nodes/llm)

다음 단계는 JSON 문자열을 받으므로 중간 Code 노드에서 객체를 `json.dumps`로 직렬화합니다. `workflow-setup.md`에 입력·출력 연결과 전체 코드를 제공합니다. 정상 메모와 담당자가 없는 메모에서 빈 배열·빈 문자열이 유지되는지 확인합니다. 필드가 필수라는 이유로 모르는 담당자를 채우면 잘못된 결과입니다.

**남길 결과·완료 판단:** 실제 LLM 노드의 스키마 설정, 생성된 객체와 다음 단계에 전달한 값입니다. 지원되지 않는 모델에서는 가능한 모델을 선택하거나 이 실제 실행을 미확인으로 남깁니다.
...
Dify의 각 Output에 `payload=업무 검사.payload`를 연결하고 Workflow를 실행 가능한 상태로 게시합니다. 앱의 API Access에서 확인한 API 기본 주소와 키를 IDE의 비공유 설정 `DIFY_API_URL`, `DIFY_API_KEY`에 넣습니다. 주소는 보통 `/v1`까지이며 호출자가 `/workflows/run`을 붙입니다. API 키가 들어 있는 설정은 AI에 공유하지 않습니다.

IDE에서 `meeting_plugin/workflow_client.py`를 기본 실행하면 제공 추출 대역과 메모리 DB로 저장·보류·재전달을 봅니다. `--live`로 실행하면 실제 Dify Workflow를 호출하고 원문과 작업을 표시합니다. 내용을 확인해 `save`를 입력한 초안만 `.local/meeting-drafts.sqlite`에 저장합니다. 같은 ID·같은 내용이면 `already_saved`, 같은 ID의 다른 내용이면 `revision_required`로 보류합니다. 이 ID는 저장 중복 방지에 쓰며 모델 호출 자체를 캐시하지는 않습니다.
...
**남길 결과·완료 판단:** 실제 Workflow의 구조화 결과가 검사·보완·사람 확인·초안 저장까지 이어지는 결과와 설계 이유입니다. 실제 저장은 확인한 초안을 대상으로 수행하며 가짜 승인 콜백을 사람의 승인 기록으로 쓰지 않습니다.

## 완료 기준

- LLM 노드의 구조화 출력을 설정하고 실제 결과를 Plugin·분기·호출 앱에 전달했습니다.
- 원문 대조와 사람 확인 뒤 초안 저장을 연결하고, 보류·재전달·내용 변경·독립 항목 실패의 결과를 해석했습니다.
