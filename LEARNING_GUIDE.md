...
작은 업무 요구에 맞는 구성을 AI와 설계하고, 구현하여 실제 결과를 판단하는 과정입니다. Git·테스트·HTTP·JSON에 익숙한 개발자를 대상으로 합니다. AI가 제안한 목적·범위·기준과 구현을 검토하고 수정하는 것도 학습입니다. 이 과정에서 프로젝트에 적용할 기본 설계·구현 경험을 확인하고, 이후 여러 프로젝트에서 더 다양한 요구와 운영 조건으로 넓혀 갑니다. 빈 문서를 혼자 설계하거나 예제 코드를 전부 다시 작성할 필요가 없습니다.
...
| 2 | Skill Creator로 제작하고 다중 제보 처리로 확장 | 출처·상충·누락에 맞는 규칙 구성과 실제 사용 |
| 3 | 자기 MCP 조회·견적·초안 저장 기능 제작 | 입력·결과·승인·저장 범위·실패 처리 선택과 실제 Host 사용 |
| 4 | 자기 요구를 포함한 작업의 위임·통합 | 수정 범위·순서·반환 약속을 선택해 적용 |
| 5 | 하네스 구성과 배치 누락 문제 해결 | 완료 근거를 검사하고 지침·실행 경로에 연결해 재사용 |
| 6 | 모델 기초·SDK 호출 구조와 자기 Tool 요구 선택 | 맥락·형식·사실의 차이, 업무의 입력·결과·실패 처리 이해 |
| 7 | 프레임워크를 이해하고 Tool·상태 흐름 구성 | 보충·정정·분기·종료를 설계하고 실제 모델과 연결 |
| 8 | RAG와 의미 검색·문서 갱신 구성 | 근거·접근 범위를 유지하며 추가·개정·제외 후 재확인 |
| 9 | 업무 확정과 사람 보완을 처리하는 Dify Workflow 구성 | 모델·규칙·사람 판단을 배분하고 분기·재처리 실행 |
| 10 | 정책·주문 문의에 필요한 구성 선택 | 기존 RAG·Tool을 필요한 경로로 연결하고 결과 판단 |
| 11 | 기존 결과를 사용하는 AI 앱 완성 | 입력→검색·업무 함수→실제 모델→API 또는 화면 연결 |
| 12 | 질문 묶음 등 후속 요구 적용과 새 세션 확인 | 부분 실패·호출 범위 설계와 재현 가능한 전달 상태 |
...
핵심 제작 활동에서는 해설을 읽은 뒤 **요구 → 구성 선택 → 적용 → 동작 확인**을 이어갑니다. 교재는 업무 조건과 필요한 입력을 제공하고, 학습자는 AI와 어떤 부분을 모델·일반 코드·도구·설정·사람의 판단에 맡길지 선택합니다. 같은 예제의 해결 방법을 복사한 것과 새로운 조건에 맞게 구성을 결정한 것을 구분합니다. 기존 구성이 이미 적합하다면 실제 근거로 유지할 수 있으며, 모든 파일을 바꿀 필요는 없습니다.

예를 들어 승인한 초안만 저장해야 한다는 업무 조건은 교재에 명시합니다. 승인과 내용의 연결을 어디서 확인하고, 저장 실패를 어떤 결과로 전달하며, 같은 요청이 다시 오면 어떻게 처리할지는 AI의 구성안을 비교해 결정합니다. 선택한 코드·설정에서 정상·거부·실패 결과를 확인해야 설계의 적용까지 마친 것입니다. 설계 이유를 적은 문서만 있거나 AI가 “완료”라고 한 것으로 대체하지 않습니다.
...
기존 과정에서 학습한 결과는 보존합니다. 개정으로 필수가 된 활동은 현재 코드·설정·실행 근거를 대조하고 부족한 부분만 보충합니다. 예를 들어 5주차에 있다면 A·B의 현재 상태를 확인해 이어가며, 이전 주차의 초안 저장 등 추가 활동은 해당 주차의 보충 안내로 돌아갑니다. 동기화는 현재 위치를 바꾸거나 Day 완료를 선언하는 작업이 아닙니다. 기존 기록을 취소하지 않으며, 새로 확인할 역량이 자동 완료되는 것도 아닙니다.
...
이번 주에는 제공된 MCP를 연결해 자료를 조회한 뒤, 필요한 조회·계산과 사람이 확인한 로컬 초안 저장을 같은 서버에 만듭니다. 기존 서버·SDK와 Codex로 구현하고, 입력·결과·접근 범위·승인·실패 처리를 선택해 실제 동작을 확인합니다.
학습 목표는 연결·호출·답변을 구분하고, 업무 요구를 MCP 기능으로 구성해 사용하고 고칠 수 있게 되는 것입니다. 기존에 사용하거나 구상한 포트폴리오가 있으면 동등한 조회·새 기록 생성 업무로 바꿀 수 있습니다. 개인 프로젝트가 없으면 이 상품 서버를 그대로 사용합니다. 실제 주문·외부 전송·원격 인증은 이번 제작 범위에 포함하지 않습니다.
...
**오늘은 자기 작업에 필요한 조회 기능을 구현하고 Inspector에서 실행합니다.** 예를 들면 예산과 재고 조건으로 상품 후보를 찾는 Tool입니다. 아래 구조 해설을 읽고 이어지는 **「자기 요구를 조회 기능으로 만들기」**의 과제를 수행합니다. AI와 기존 SDK를 자유롭게 활용할 수 있습니다. Day 4에서는 만든 기능이 요구대로 동작하는지 검증하고, Day 5에서는 후속 구매 검토·새 초안 저장을 이어 구현한 뒤 다른 입력과 변경된 자료로 다시 사용합니다.
...
현재 서버와 계산 기능을 AI에게 보여 주고 조회·미리보기·저장의 구성을 제안받습니다. 학습자는 새 Tool로 나눌지, 계산·저장 결과를 어떻게 연결할지, 사람의 확인을 어느 실행 경로에서 받을지 결정합니다. 아래는 정해진 업무 기준이며 함수 이름·입력 필드·파일 구성의 정답은 아닙니다.
...
현재 서버에서 이를 지킬 수 있는 작은 구성안을 제안해 주세요.
어떤 도구·입력·결과를 쓰며 사람 확인과 서버 검사가 각각 어디서 이뤄지는지
정상 저장과 같은 식별자의 다른 내용 사례로 설명해 주세요.
내가 선택한 방식을 기존 서버·SDK와 표준 파일 기능으로 구현하고 실제 MCP에서 사용하게 해 주세요.
기존 조회·계산은 유지하고, 연습 초안만 사용해 미승인·재요청·충돌·실패도 확인하세요.
새 검증기나 별도 설계 보고서는 만들지 말고 기존 테스트와 mcp-use.md에 연결해 주세요.
```
...
**남길 결과·완료 판단:** 구매 검토와 새 초안 저장이 반영된 코드·필요한 테스트를 남깁니다. 기존 `mcp-use.md`에서 선택한 도구 구성·승인과 내용의 연결·재요청·충돌·실패 처리의 이유와 실제 결과를 찾을 수 있게 합니다. 기존 조회·계산, 바뀐 자료, 승인한 새 초안과 필수 보존 사례를 확인했으면 Day 5를 마칩니다. Inspector에서 직접 저장을 선택해 완료했다면 Host를 통한 쓰기 승인·실행은 아직 미확인이라고 구분합니다. 제공 구매 검토 업무로 충분하며 별도 개인 프로젝트를 추가할 필요는 없습니다.
...
Day 5의 필수 제작은 사람이 확인한 **새 초안 생성**과 재요청·충돌·실패 시 자료 보존까지입니다. 기존 초안을 함께 편집하거나 중단된 작업을 복구하는 업무가 필요하면 같은 서버에 다음 요구를 이어 적용합니다. 새 서버나 별도 프로젝트를 만들 필요는 없습니다.

> 저장한 초안을 다시 검토해 수정할 수 있어야 합니다. 미리보기 이후 다른 사람이 변경했다면 덮어쓰지 말고 알려 주세요. 프로그램이 중단돼도 어떤 요청이 저장됐는지 구분하고, 취소할 때 이후의 수정까지 지우면 안 됩니다.

AI와 버전 확인·수정 충돌·저장 결과 기록·취소 범위를 정합니다. 같은 요청 ID와 다른 내용을 구분하고, 재시작 뒤에도 저장 상태가 이어지는지 확인합니다. 파일 교체가 한 번에 끝난다는 사실만으로 버전 확인부터 기록까지의 경쟁이 해결되지는 않습니다. 실제로 동시 요청을 처리한다면 기존 잠금·저장 도구를 사용해 검사와 반영 범위를 연결하고 해당 조건을 테스트해야 합니다.

확인에는 연습 초안만 사용합니다. 미리보기 뒤 직접 수정한 초안의 보존, 재시작 뒤 재요청, 저장 중단과 재시도, 이후 수정이 있는 초안의 취소 거부를 선택한 정책에 맞춰 확인합니다. 선택한 기능의 코드·실행 결과와 확인하지 않은 동시성·원격 인증 범위는 기존 `mcp-use.md`에 이어 남깁니다. 여러 서버와 분산 저장, 외부 서비스 운영은 이 로컬 심화를 완료하기 위한 조건이 아닙니다.
...
목표는 **역할별 관점이 어떤 검토에 도움이 되는지 이해하고, 업무 요구에 맞게 맡길 범위와 순서를 정해 두 변경을 함께 사용하는 것**입니다. 제공 예제의 제목·우선순위 변경에 두 결과를 함께 봐야 하는 긴급 티켓 조건을 더하고, 같은 프로젝트에서 책임과 통합 순서를 선택해 구현합니다. 기존 포트폴리오가 있으면 같은 종류의 공동 동작과 기존 기능 보존이 필요한 작은 요구로 바꿀 수 있습니다.
...
Codex에게 현재 코드에 맞는 담당·반환 약속·순서의 후보를 제안하게 합니다. 학습자는 독립 구현이 가능한 부분, 다른 작업이 알아야 할 결정, 공통 부분을 바꿀 담당과 시점을 선택합니다. 기본 두 변경을 나누어 구현하는 연습은 유지하지만, 후속 조건까지 모두 동시에 수정하게 할 필요는 없습니다. 선택한 요구와 범위·순서를 기존 프로젝트 `README.md`에 짧게 반영해 다음 작업도 같은 결정을 읽을 수 있게 합니다.

```text
기본 제목 정리·P0 표시와 위의 제목 없는 긴급 티켓 요구를 현재 코드에 적용하려고 합니다.
입력이 두 함수와 최종 표시를 거치는 경로에서 어느 부분이 함께 알아야 하는 정보를 갖는지 설명해 주세요.
현재 함수를 재사용할 담당 범위·반환 약속·먼저 정할 결정과 통합 순서의 후보를 제안해 주세요.
제안한 방식을 기본 입력과 제목 없는 P0/P1 입력으로 설명하고, 내가 선택한 내용을
기존 README의 요구·담당·순서에 짧게 반영해 주세요. 아직 구현·테스트 코드는 수정하지 마세요.
```
...
**남길 결과·완료 판단:** 두 Worktree의 코드·테스트 변경을 남기고, 노트에 각 폴더·시작 커밋·검사 결과를 연결합니다. 같은 시작 코드에서 선택한 범위대로 수정했고 각 작업이 맡은 입력·기대 결과를 단독 검사로 확인했으면 Day 3을 마칩니다. 공통 조건을 Day 4에 처리하기로 했다면 그 담당과 남은 확인을 구분하며, 단독 검사만으로 긴급 조건의 통합 성공을 선언하지 않습니다.
...
**남길 결과·완료 판단:** 원본 작업에 통합한 두 변경과 공동 조건의 구현·커밋을 남기고, 노트에 통합 후 전체 검사·앱 실행 결과를 연결합니다. 기본 입력의 `[긴급] 로그인 오류`, 제목 없는 P0의 `[긴급] 담당자 확인 필요`, 같은 빈 제목의 P1과 기존 미등록 값 동작을 함께 확인했으면 Day 4를 마칩니다. 실제 포트폴리오를 사용했다면 선택한 동등 요구의 공동 동작과 기존 기능 보존을 확인합니다.
...
- [ ] 기본 두 변경과 공동 조건의 담당 범위·반환 약속·실행 순서를 AI의 제안과 실제 코드에 근거해 선택하고 각 작업이 읽을 요구에 반영했습니다.
...
6~7주차는 하나의 주문 문의 도우미 또는 같은 역할의 자기 기능을 이어 만드는 과정입니다. 이번 주에는 제공 SDK 예제로 모델 요청·함수 실행·결과 전달을 읽고, **번호가 없는 문의, 없는 주문, 조회 서비스 실패를 어떻게 다르게 처리할지 AI와 정해 로컬 업무 함수에 적용**합니다. 7주차에는 SDK 또는 프레임워크 중 한 경로를 선택해 실제 모델에 연결하고 보충·정정 입력을 이어 처리합니다.
...
- 선택한 계약을 작은 로컬 업무 함수에 적용하고 정상·실패·자료 변경 결과를 확인합니다.
...
Day 1의 주문 업무 또는 같은 역할의 자기 요구를 Codex와 구체화합니다. 먼저 정상 요청과 세 종류의 실패에 대해 함수 입력·결과, 사용자에게 보여 줄 행동을 제안받습니다. 학습자는 필요한 식별자, 공개해도 되는 결과, 되물어야 할 정보와 중단할 상황이 목적에 맞는지 판단합니다.
...
이어서 기능 검사의 임시 데이터에서 `O-100`의 배송 상태를 `배송 준비`에서 `배송 완료`로 바꾸어 다시 조회합니다. 변경 전에 어느 함수 결과가 달라질지 예상하고 실제 반환값과 연결된 답변을 대조합니다. 임시 데이터는 검사 뒤 원래 값으로 돌아오게 하여 이후 주차의 기본 자료 `O-100: 배송 준비`, `O-200: 배송 중`을 유지합니다. 이 변경은 입력·실패 설계를 적용한 뒤 값의 경로를 확인하는 활동이며, 자료 값만 바꾼 것을 자기 Tool 설계·제작 전체로 판단하지 않습니다. 임시 변경 검사의 기대값만 선택한 값에 맞추고 기존 오류·결과 전달 검사는 보존합니다.

실패하면 인자 검사→업무 함수→결과 전달→답변 중 처음 어긋난 위치를 고칩니다. 고정 응답에서는 선택 ID와 답변 필드도 고정돼 있음을 확인하고, Fake 응답을 바꾼 결과와 실제 모델의 판단을 구분합니다.

**남길 결과·완료 판단:** 누락·부재·서비스 실패의 실제 결과와 자료 변경 전후의 예상·출력을 기존 노트에 연결합니다. 실패를 성공으로 설명하지 않고 변경된 자료가 함수에 반영되면 로컬 확인을 마칩니다. 실제 모델 연결 미확인은 별도로 남깁니다.
...
- [ ] 자료 변경 전 예상과 실제 함수 결과를 대조하고 필요한 기존 검사를 유지했습니다.
...
입출력·상태·분기안과 코드는 AI에게 제안받고 학습자가 요구와 실제 결과로 판단합니다. 개인 프로젝트가 없으면 합성 주문 `O-100: 배송 준비`, `O-200: 배송 중`, 없는 주문 `O-999`를 사용합니다. 6주차에서 자료를 바꿨다면 현재 값을 확인해 기대 결과를 그 자료에 맞추거나, 서로 다른 주문임을 비교할 수 있는 두 상태를 정합니다. 자기 프로젝트로 바꾸면 식별자 누락 뒤 보충, 식별자 정정 시 이전 결과 무효화, 반복 상한과 조회 실패가 필요한 업무로 대응합니다.
...
- 같은 Tool에 상태·조건 분기를 연결하고 번호 보충, 번호 정정 시 이전 결과 무효화, 재질문 상한을 실제 입력으로 확인합니다.
...
| 5 | 번호 정정 요구 반영과 재사용 | 이전 조회·초안 무효화, 새 번호의 사실과 기존 동작 유지 |
...
Day 1에서 본 프레임워크와 6주차 SDK 중 **구현을 이어 갈 방식 하나**를 선택합니다. SDK의 연결이 이해하기 쉽고 충분하면 그대로 사용해도 됩니다. 프레임워크의 도구 등록·메시지 관리가 도움이 되면 `framework_lab/`에서 구현합니다. 자기 Tool이 이미 작동한다면 새로 만들지 말고 아래 변경 지점과 요구가 맞는지 검토합니다. 프레임워크로 옮긴다면 6주차의 업무 함수 자체를 이동하거나 가져와 등록하며 같은 조회 로직을 두 곳에 복제하지 않습니다.
...
1. AI와 상태의 항목·갱신 시점, 입력 부족·조회 성공·없는 주문·서비스 실패·보충 상한의 다음 행동을 정합니다. 대화문에서 성공 여부를 다시 추측하지 않도록 실제 함수 반환값을 상태로 연결합니다.
...
실제 문의와 예상 상태 변화로 제안해 내가 이유를 판단하게 해 주세요.
선택한 구성에 적용하고, 입력 뒤 같은 문의의 상태를 유지하는 실행 진입점도 연결하세요.
기존 SDK 또는 프레임워크의 모델·도구 호출을 사용하고 범용 실행 엔진은 만들지 마세요.
통제된 모델·함수로 분기를 확인한 결과와 실제 모델의 해석·답변을 구분해 주세요.
같은 노트에 코드 위치·실제 상태 변화와 다음 실제 실행 방법을 연결해 주세요.
```
...
현재 상태·조회 결과·초안의 연결을 읽고, 번호 변경 시 무효화할 값과 다시 실행할 단계를 제안하세요.
내가 선택한 방법을 기존 코드에 적용하고 원래 문의 내용은 유지해 주세요.
같은 Tool을 재사용하며 새 조회 실패 때도 O-100의 결과를 성공값으로 되살리지 마세요.
보충 횟수와 종료 조건, 기존 정상·정보 부족·없는 주문·서비스 실패 처리를 유지하세요.
아래 사례의 실제 상태·함수 결과·답변을 확인하고 기존 노트에 이어 연결해 주세요.
```
...
**남길 결과·완료 확인:** 변경 요구→선택한 무효화·분기 방식→실제 상태와 새 주문 결과를 연결합니다. 보충·정정·상한과 기존 정상·실패가 함께 유지되고 실제 모델의 문의 처리를 확인하면 6~7주차 제작을 마칩니다. 이미 쓸 수 있는 코드·실행 기록을 기존 README와 노트에 연결하면 충분합니다.
...
- [ ] 상태·처리 단계·조건 분기의 초안을 AI와 선택하고 선택 이유를 기존 프로젝트에 적용했습니다.
- [ ] 번호가 없으면 질문하고 보충 뒤 같은 문의로 조회하며, 번호를 받지 못하면 정한 상한에서 보류하는 실제 경로를 확인했습니다.
- [ ] `O-100`에서 `O-200`으로 정정하거나 자기 업무의 식별자를 정정했을 때 이전 조회 결과·초안을 무효화하고 새 대상으로 처리했습니다.
...
자기 자료가 없으면 제공 합성 정책을 사용합니다. 개인 프로젝트가 있다면 현재 버전·접근 범위·근거 없음이 중요하고 문서를 추가·개정·제외하는 같은 역할의 요구로 바꿉니다. AI가 기존 제품·SDK로 연결과 수정안을 만들고, 학습자는 청킹·검색 필터·갱신 방식과 실제 근거·답변을 판단합니다. 검색 엔진이나 임베딩 모델 내부를 재구현하지 않습니다. 답변 생성은 대화형 Codex 또는 애플리케이션 API 중 한 경로를 선택하되, 임베딩과 검색 저장소의 실제 사용은 별도로 확인합니다.
...
- 업무 요구에서 청킹·현재 자료와 접근 필터·임베딩 모델·검색 저장소의 구성을 AI와 선택합니다.
- 학습된 임베딩과 실제 검색 저장소를 연결하고 표현이 달라진 질문을 어휘 기준선과 비교합니다.
...
| 3 | 학습된 임베딩과 저장소를 실제 연결 | 같은 질문의 어휘·의미 검색 후보와 실제 생성 답변 비교 |
...
앞의 코드 해설에서 보았듯 초기 `pipeline.py`는 질문에 “과거”가 들어가면 보관 자료를 허용하고, `answering.py`는 같은 문서의 한 조각만 선택합니다. 따라서 이번 업무의 현재 범위와 여러 조건 전달에 맞는지 판단해야 합니다. AI에게 현재 결과를 보여 주고 **무엇을 검색 단위로 삼을지, 접근·현재 자료를 어디서 제한할지, 답변에 어떤 근거를 얼마나 전달할지** 제안받습니다. 현재 청킹으로 조건이 함께 유지된다면 그대로 사용할 수 있습니다. 선택은 원문·검색 결과·후속 변경에 근거하며 모든 설정을 바꿀 필요는 없습니다.

```text
위의 현재 환불 안내와 표현 차이·정책 갱신 요구를 기존 rag_lab에 적용하려고 합니다.
초기 어휘 검색의 질문·후보·sources·답변을 보고 필요한 변경과 유지할 부분을 제안해 주세요.
문서 조각·메타데이터·임베딩·저장소·생성의 역할을 이 입력의 경로로 설명해 주세요.
현재 자료와 접근 범위를 제한할 위치, 함께 전달할 조건, 검색 점수와 보류의 판단을
내가 선택할 수 있게 실제 원문과 예상 근거로 연결해 주세요.
임베딩/저장소는 기존 제품·SDK를 사용하고 내부 검색 엔진을 만들지 마세요.
요구와 선택 이유는 ../rag-note.md에 이어 적고 Day 3의 실제 연결로 진행해 주세요.
```
...
별도 운영 환경이 없다면 **로컬 Chroma의 디스크 저장 + API 임베딩**을 시작 경로로 선택할 수 있습니다. 예를 들어 `text-embedding-3-small`을 명시해 문서·질문에 같은 임베딩 모델을 사용합니다. 임베딩 모델은 답변 생성 모델과 역할이 다르므로 `OPENAI_MODEL`과 혼동하지 않게 이름을 설정합니다. [OpenAI 임베딩 사용법](https://developers.openai.com/api/docs/guides/embeddings)
...
기존 rag_lab의 어휘 검색을 보존하고 제품의 공식 SDK로 실제 임베딩·저장·조회를 연결해 주세요.
설치된 버전과 공식 예제를 확인하고 필요한 패키지·pyproject.toml·uv.lock을 함께 반영하세요.
문서와 질문에 같은 모델·차원을 사용하고 .local/rag/ 안의 고정 경로에 실습 인덱스를 저장하세요.
...
같은 질문의 어휘 기준선·새 검색 후보·sources·실제 생성 답을 기존 노트에서 대조하게 해 주세요.
```
...
**남길 결과·완료 확인:** 실제 모델 이름과 임베딩 실행, 저장소의 저장·조회, 기존 sources까지 이어진 코드·의존성과 질문 결과를 연결합니다. 실제 검색 근거로 생성한 답을 원문과 대조해야 Day 3을 완료합니다. 설치·네트워크·모델 접근이 막혔으면 원인과 실제 시도·남은 명령을 남기고 해당 연결은 `NOT_VERIFIED`로 둡니다. 도구를 사용하지 않기로 결정한 글이나 모의 임베딩만으로 필수 연결을 완료하지 않습니다.
...
AI와 **전체 재색인 또는 문서 ID별 변경분 갱신 중 하나**를 선택합니다. 작은 문서 묶음이면 전용 실습 컬렉션을 현재 허용 자료로 다시 만드는 방법이 단순합니다. 변경분만 다룬다면 문서·버전·조각 ID의 대응을 유지하고 폐기한 이전 조각을 제거하거나 조회 조건에서 제외해야 합니다. 다른 컬렉션이나 개인 자료를 일괄 지우는 명령은 사용하지 않습니다. 같은 임베딩 모델·차원으로 만든 인덱스를 재사용하고 모델을 바꾸면 별도 컬렉션 또는 재색인이 필요한지 확인합니다.

```text
위 순서의 가상 정책 개정·추가·제외를 같은 RAG에 반영해 주세요.
원본 knowledge_base를 보존하고 내가 사용할 복사본·실습 저장소 경로를 확인하세요.
현재 자료 규모에서 전체 재색인과 ID별 갱신의 차이를
45일 개정·2영업일 추가·철회 후 남아서는 안 될 조각으로 설명해 내가 선택하게 해 주세요.
선택한 방식으로 기존 색인/질의 진입점을 수정하고 정확한 실행 명령을 알려 주세요.
각 변경 뒤 실제 저장소를 갱신해 같은 질문의 후보·sources·생성 답변을 원문과 대조하세요.
...
- [ ] 제공 또는 자기 업무의 현재 자료·접근 범위·조건·표현 차이·갱신 요구를 AI와 구체화하고 구성 선택 이유를 설명했습니다.
- [ ] **학습된 임베딩 모델과 검색 저장소를 실제 연결해 문서·질문 임베딩, 저장·조회와 sources 전달을 확인했습니다.**
- [ ] 같은 문서·질문의 어휘 기준선과 새 검색 후보·실제 생성 답변을 비교하고 확인한 변화와 한계를 구별했습니다.
...
AI가 입력·출력·노드·검사·보완 방식의 초안을 만들 수 있습니다. 학습자는 무엇을 모델이 추출하고 어떤 기준을 규칙으로 검사하며 어떤 사실을 사람이 결정할지 실제 메모와 결과로 선택합니다. 개인 프로젝트가 없으면 아래 회의 자료를 사용합니다. 자기 업무로 바꾸려면 정상·누락·상충·부분 보완·보류에 따라 처리 경로가 달라지는 문서 검토나 요청 접수처럼 같은 역량을 확인할 수 있는 요구를 고릅니다.
...
| 2 | 변수·노드·출력 구조를 분석하고 책임 선택 | 원문 추출·검사·사람 판단을 어디에 맡길지 선택한 이유 |
| 3 | 누락·상충을 검사하는 실제 분기 구성 | 정상은 확정 가능, 부족·상충은 검토 필요 경로로 이동 |
...
출력 필드와 노드 수는 AI의 초안을 검토해 선택합니다. 필수값·상충·보류의 검사 결과를 다음 IF/ELSE가 실제로 사용하고, 모델이 적은 제목만으로 완료를 판정하지 않아야 합니다. “담당자·기한 필수”가 맞지 않는 자기 업무라면 그 업무에서 확정 전에 필요한 정보를 정하고 같은 누락·상충·보완 경로로 확인합니다.
...
정상 메모에는 서연과 날짜가 모두 있는데 추출 결과가 비었다면 모델 입력·추출 단계에서 어긋난 것입니다. 원문부터 점검 담당자가 미정이라면 추출은 그 사실을 보존하고 다음 검사는 검토 필요로 보내야 합니다. 입력에 “민수”와 “지민”이 함께 있어도 이들이 서로 다른 일을 맡는지, 같은 일의 상충인지 관계를 읽어야 합니다. 검사 필드에 이름이 하나 있다는 사실만으로 충돌이 해결되었다고 볼 수 없습니다.

검토 이유와 원문 근거를 다음 출력까지 전달하면 사람이 무엇을 보완해야 하는지 알 수 있습니다. 반대로 `검토 필요`라는 한 단어만 반환하면 이미 확인한 날짜·업무를 버리고 처음부터 다시 묻게 됩니다. 구조화된 출력은 이러한 값을 연결하기 위한 수단이며 실제 내용 검토를 대신하지 않습니다.

```text
현재 Dify Workflow에 위의 회의 업무 요구를 적용해 주세요.
정상·누락·상충 입력에서 모델이 추출할 값, 규칙으로 검사할 기준,
사람이 결정할 사항을 실제 문장과 예상 노드 결과로 제안해 내가 선택하게 해 주세요.
선택한 필드·검사·조건 분기를 기존 Workflow에 연결하세요.
정상과 검토 필요가 실제로 다른 경로를 실행하며 확인된 작업·근거와 미해결 사유를 보존하게 하세요.
빈 입력은 모델 호출 전에 처리하고 형식 오류를 정상 빈 결과로 바꾸지 마세요.
위 사례를 Test Run으로 실행해 각 노드 값과 분기 결과를 대조하고 필요한 부분만 수정해 주세요.
...
**남길 결과·완료 확인:** 정상·누락·상충·빈 입력에서 실제 검사 결과와 실행된 분기를 연결합니다. 확인된 작업을 보존하고 부족·상충을 지어내지 않으며 빈 입력에서 모델이 호출되지 않으면 Day 3을 마칩니다. 결과가 다르면 처음 어긋난 변수·추출·검사·분기·출력을 고친 뒤 같은 입력으로 확인합니다.

### Day 4 — 사람의 보완·보류를 받아 같은 업무 이어 처리하기

검토 경로를 실제로 사용합니다. Day 3의 누락·상충 결과에서 사람이 필요한 정보를 보완하거나 보류하면 같은 업무를 다시 검사하도록 연결합니다. AI에게 가능한 입력 연결을 제안받아 사용 환경에 맞는 한 방식을 선택합니다.
...
접근성 점검에 담당자·기한이 모두 없던 결과에 “담당자는 민수입니다”만 들어오면 담당자 한 항목만 보완됩니다. 기한을 이전 작업의 9월 16일에서 가져오거나 그럴듯한 날짜로 채우면 새 사실을 만든 것입니다. 보완 반영 후 같은 필수정보 검사를 다시 실행해야 기한 질문이 남습니다. 반대로 “담당자는 민수, 기한은 2026-09-18입니다”라면 그 두 값을 반영해 다시 검사할 수 있습니다.

상충은 사람에게 무엇이 충돌하는지 보여 주고 받아들일 사실을 선택받아야 합니다. 예를 들어 “이 접근성 점검 담당자는 지민으로 확정합니다”라는 보완을 받으면 원문 속 민수·지민 충돌을 어떤 권한과 규칙으로 해소할지 정합니다. 이번 실습의 검토자는 학습자이며 모델이 그 결정을 대신하지 않습니다. 단순히 마지막 문장을 우선하는 규칙을 모든 자료에 적용한다고 가정하지 않습니다.
...
화면상의 기대 순서만 적지 말고 원문·보완·검사 결과·실행된 분기·출력을 기존 노트에 연결하세요.
```
...
**남길 결과·완료 확인:** 한 가지 보완 방식을 실제 연결하고 누락·상충을 사람이 해결한 뒤 검사·분기가 달라지는지, 보류 뒤 실제 입력으로 다시 처리할 수 있는지 확인합니다. 노트에는 원문·보완·검사 결과·출력과 선택 이유를 연결합니다. Human Input을 선택하지 않아도 이 재처리 역량을 확인하면 됩니다.
...
**남길 결과·완료 확인:** 부분 보완 때 남은 문제를 유지하고, 충분한 보완과 정정 뒤 재검사하며, 기존 정상·빈값·상충·보류 동작이 유지되는지 확인합니다. 최종 export와 실제 결과를 연결하면 마칩니다. 새 환경 import·실행을 하지 않았다면 그 범위만 미검증으로 남깁니다.
...
- [ ] 제공 또는 자기 업무의 확정 가능·검토 필요 기준과 모델 추출·규칙 검사·사람 판단의 책임을 AI와 선택하고 이유를 설명했습니다.
...
- [ ] 부분 보완 뒤 남은 질문을 유지하고, 충분한 보완·정정 후 이전 값이 섞이지 않게 재검사했습니다.
...
이번 주의 기본 후속 요구는 “상담원이 정책 질문과 주문 현황 질문을 한 곳에서 처리한다”입니다. 직접 분기, Workflow, agent 가운데 요구에 맞는 방식을 선택해 구현합니다. agent를 선택하는 것이 정답은 아니며, 모든 기술을 최종 앱에 넣을 필요도 없습니다. 선택 이유만 쓰거나 빈 입력 검사만 추가한 것으로 이번 기능 적용을 마치지 않습니다.
...
| 2 | 실제 코드·결과에서 역할과 변경 부담 비교 | 재사용 위치·차이·현재 선택의 이유 |
...
설계안을 내가 판단할 수 있도록 정상·정보 부족·조회 실패의 예상 결과를 보여 주세요.
별도 설계 보고서를 먼저 완성하게 하지 말고 기존 노트에서 구현으로 이어 주세요.
```
...
기존 코드에서 실제로 얻은 근거가 충분하면 두 앱을 완성해 비교하지 않습니다. 차이가 불명확할 때만 같은 작은 입력으로 확인합니다. 코드로 읽은 동작, 실제 실행, 그 효과에 대한 해석을 같은 노트에서 구분합니다.

**남길 결과·완료 확인:** 재사용할 함수·자료·모델 연결과 새 분기의 위치를 짚고, 현재 방식 또는 대안을 선택한 이유와 아직 확인할 차이를 설명합니다.

### Day 3 — 선택한 구성으로 업무 기능 구현하기

Day 2에서 고른 한 방식으로 기존 앱을 수정합니다. AI에게 입력의 종류를 구분할 위치, 조회 결과를 전달할 형태, 처리 불가 상태와 호출 상한을 제안받고 채택한 구성을 구현하게 합니다. 별도 설계 문서를 먼저 완성하거나 SDK·검색기 내부를 다시 만드는 과제가 아닙니다.
...
먼저 이번 변경의 입력·결과·실패 계약을 짧은 예로 설명하고 구현을 진행해 주세요.
모델 대역으로 확인할 분기와 실제 모델로 확인할 요청을 구별하고,
기존 테스트에서 변경에 필요한 정상·실패 사례를 확인해 주세요.
...
노트에 새 요구, 유지할 구성·제외할 구성, 변경한 위치, 실제 결과, 선택을 재검토할 상황을 정리합니다. 도구를 하나도 새로 도입하지 않았더라도 기존 방식으로 새 기능을 구현하고 필요한 검증을 했다면 타당한 결론입니다. Skill·Workflow를 보조로 썼다면 앱에서 맡은 실제 역할을 적습니다.

11주차에서 연결할 함수나 기존 API·화면 진입점과 필요한 모델·자료 설정을 짚습니다. 실제 모델 확인이 남았다면 그 상태를 그대로 이어 처리하고, 설명만 완료해 앱 통합을 마쳤다고 표시하지 않습니다.

**남길 결과·완료 확인:** 새 표현과 기존 사례의 결과를 근거로 구성을 수용하고, 다음 사용 경로에서 호출할 기능·설정·남은 한계를 찾을 수 있으면 끝납니다.
...
- [ ] 실제 코드·설정·결과를 근거로 현재 방식과 필요한 대안의 차이·변경 부담을 설명했습니다.
...
- [ ] 선택을 바꿀 조건과 11주차에 연결할 기능·실행 위치를 남겼습니다.
...
이번 주에는 10주차에서 적용한 기능을 **사용자의 입력부터 실제 모델과 업무 결과까지 이어지는 AI 앱**으로 완성합니다. 진행 중인 포트폴리오나 구상한 앱이 있다면 기존 스택·화면·API에 필요한 연결을 더합니다. 개인 프로젝트가 없다면 10주차의 환불 정책·주문 문의 도우미에 로컬 `POST /answer`를 연결합니다.
...
| Day | 할 일 | 남길 결과 |
|---:|---|---|
| 1 | 10주차 기능을 API 또는 기존 화면에 연결 | 동작하는 진입점과 첫 실제 입력·답변·근거 |
| 2 | 사용 경로를 따라 결과 계약·근거 선택 확인 | 역할·설정·수정 위치와 수용 판단 |
| 3 | 입력 검사와 깊은 오류 전달 경로 완성 | 업무 보류와 서비스 실패가 구분된 실제 응답 |
...
API나 화면에서 실제 모델까지 자동으로 이어지게 구현하고,
사용자에게 필요한 답변·근거·상태만 반환하도록 제안해 주세요.
부족한 계약은 짧은 입출력 예로 내가 검토할 수 있게 설명하며 구현으로 이어 주세요.
...
**남길 결과·완료 확인:** 입력에서 기존 기능·실제 모델·사용자 응답이 자동으로 이어졌는지 확인하고 진입점·자료·실제 결과를 기록합니다. 고정 응답이나 검색만 연결한 경우 실제 모델 통합 확인이 남습니다.
...
정책 근거는 충분한데 제공자가 출력을 끝내지 못한 상황에도, 최종 결과만 보면 자료가 부족했다고 읽힙니다. API에서 예외만 잡는 것으로는 이 경로가 고쳐지지 않습니다. 중간 함수가 정상 업무 상태로 바꾸어 반환했기 때문입니다.

AI와 함께 모델의 미완성·거부·파싱 실패를 별도 상태·오류 코드 또는 명시적 예외로 전달하게 수정합니다. 모델이 근거·조건 부족 때문에 의도적으로 보류한 응답은 업무 결과로 유지합니다. `run()`이 서비스 원인을 다시 보류 문구로 덮지 않는지도 확인합니다. 개인 스택에도 같은 경계가 있다면 해당 provider→생성 함수→업무 함수→API/화면을 추적합니다.
...
두 번째 확인은 중간 변환이 오류를 숨기는지 검사합니다. 제공자의 실제 가용성·시간 초과를 검증하는 것은 아니며, 오류를 만들려고 유료 호출을 반복하지 않습니다. 실제 모델의 정상 요청은 Day 1·4에서 별도로 확인합니다.
...
**남길 결과·완료 확인:** 입력 거절·업무 보류·서비스 실패의 응답이 구분되고, provider 대역→생성→업무 함수→사용 경로에서 원인이 보존되는지 확인합니다. 코드·설정·수정 전후 결과를 기존 노트에 연결합니다.
...
- [ ] 기존 포트폴리오 또는 10주차 기본 도우미에서 사용자·입력·결과·실패 계약을 정했습니다.
...
- [ ] 잘못된 입력·업무 보류·서비스 실패를 구별하며 provider 응답 대역부터 최종 안내까지 오류 전달을 확인했습니다.
...
이번 주에는 11주차 앱을 새 세션에서 다시 사용하고 **아직 구현하지 않은 후속 업무 요구**를 같은 프로젝트에 반영합니다. 기본 요구는 “상담원이 질문 묶음을 보내도 일부 실패 때문에 나머지 결과를 잃지 않는다”입니다. 입력과 결과의 연결, 부분 실패, 호출 상한을 AI와 설계하고 구현·실행한 뒤 안내와 전달 상태를 정리합니다.
...
부분 실패는 묶음 중 일부 작업만 완료하지 못한 상태입니다. 세 질문 중 두 번째 모델 호출이 실패해도 첫 번째 결과를 보존하고 세 번째 질문을 처리할 수 있습니다. 전체 JSON 형식이 잘못됐거나 묶음 한도를 넘었다면 실행 전에 요청 전체를 거절하는 편이 맞을 수 있습니다. 어느 오류가 항목 하나에 속하고 어느 오류가 묶음 전체를 막는지 정하는 것이 앱 설계입니다.
...
| 항목 | 설계하고 확인할 기본 요구 |
|---|---|
| 사용 장면 | 로컬 상담원이 허용된 합성 정책·주문 자료에 질문 여러 개를 보내고 결과를 함께 확인 |
...
| Day | 할 일 | 남길 결과 |
|---:|---|---|
| 1 | 새 세션에서 기존 앱 재현 후 후속 요구 설계 | 재현 결과와 선택한 입력·결과·실패 계약 |
| 2 | 새 기능 구현, 일부 실패·상한 확인 | 변경 코드와 대역을 이용한 경계 검증 |
...
정책·주문 질문은 각각의 근거와 답변으로 연결하고, 마지막 질문은 관련 자료가 없으면 업무 보류로 남아야 합니다. 두 번째 항목에 서비스 실패가 발생해도 첫 번째 결과와 세 번째 항목의 처리 상태를 확인할 수 있어야 합니다. 실패는 Day 2의 대역으로 의도적으로 재현하고 실제 유료 연결을 망가뜨려 만들지 않습니다.
...
설계 이유를 작은 입출력 예로 설명해 내가 판단하도록 하고 같은 프로젝트에서 구현으로 이어 주세요.
병렬 실행·새 큐·새 프로젝트를 필수로 추가하지 말고, 기존 노트·테스트를 사용해 주세요.
```

**남길 결과·완료 확인:** 새 세션의 실제 사용 결과와 환경·막힌 위치를 남기고, 새 요구의 입력·항목 연결·부분 실패·상한 계약을 정합니다. 재현 문제가 있다면 먼저 고칠 위치를 함께 짚습니다.

### Day 2 — 기존 함수를 재사용해 구현하고 일부 실패 확인하기

AI와 선택한 설계를 기존 앱에 구현합니다. 기본 경로에서는 11주차의 단일 질문 처리 함수를 항목별로 호출하고 결과를 연결합니다. 입력 계약과 실행·오류 경계를 추가하되 정책 검색·주문 Tool·모델 호출 로직을 별도로 복제하지 않습니다. API나 화면에서도 새 기능에 도달할 수 있게 연결합니다.
...
반대로 각 항목의 호출 경계에서 처리 가능한 서비스 오류를 항목의 결과로 바꾸어 누적하면 이미 얻은 답을 보존하고 다음 유효 질문을 처리할 수 있습니다. 다만 모든 예외를 성공 답변으로 바꾸면 결함을 숨깁니다. 실패 항목에 명시적인 상태를 남기고, 실행할 수 없는 전체 요청은 별도로 거절해야 합니다. 순차·병렬 여부보다 **입력 ID·오류 경계·결과 계약**을 먼저 정하는 이유입니다.

11주차 함수가 예외 대신 서비스 오류 상태를 반환한다면 그 의미를 읽어 항목 결과로 보존합니다. `ABSTAINED`를 모두 실패로 바꾸거나 같은 실패 항목을 무조건 재시도하지 않습니다. 11주차에서 구분한 업무 의미를 유지합니다.
...
**남길 결과·완료 확인:** 새 계약을 반영한 코드와 항목 연결·부분 실패·빈 묶음·상한의 실제 검사 결과를 남깁니다. 실패 항목을 지우거나 기대를 낮춰 통과시키지 않습니다.
...
새 기능에서 둘째 항목을 실패하게 했는데 첫째 답과 셋째 보류가 반환됐다면, 해당 입력과 실패 조건에서 부분 결과가 보존된 것입니다. 그것만으로 모든 네트워크 장애나 여러 사용자 환경에서 복구된다고 주장할 수 없습니다.
...
- [ ] 기본 질문 묶음 또는 포트폴리오의 동등한 미구현 후속 요구를 같은 앱에 설계·구현했습니다.
- [ ] 입력과 결과 연결, 일부 실패 보존·계속 처리, 업무 보류와 서비스 오류 구분을 확인했습니다.
